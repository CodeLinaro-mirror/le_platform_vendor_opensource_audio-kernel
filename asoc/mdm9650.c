...
static int mdm_hifi_ctrl(struct snd_soc_component *component)
{
...
static int mdm_mi2s_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	entry = snd_info_create_module_entry(card->module,
					 "tasha",
					 card->proc_root);
	if (!entry) {
...
static struct snd_soc_dai_link mdm_auto_dai[] = {
	/* Backend DAI Links */
	{
		...
		.ignore_pmdown_time = 1,
		.ignore_suspend = 1,
		SND_SOC_DAILINK_REG(auto_i2s_rx),
	},
	{
		.name = LPASS_BE_PRI_MI2S_TX,
		.stream_name = "Primary MI2S Capture",
		...
		.ignore_pmdown_time = 1,
		.ignore_suspend = 1,
		SND_SOC_DAILINK_REG(auto_i2s_tx),
	},
};
					...
					struct snd_soc_card *card)
{
	...
	for (i = 0; i < card->num_links; i++) {
		...
		if (dai_link[i].platforms->dai_name &&
		    !dai_link[i].platforms->of_node) {
			index = of_property_match_string(cdev->of_node,
						"asoc-platform-names",
						dai_link[i].platforms->dai_name);
			if (index < 0) {
				pr_debug("%s: No match found for platform name: %s\n",
					__func__, dai_link[i].platforms->dai_name);
				ret = index;
				goto err;
			}
			...
			dai_link[i].platforms->dai_name = NULL;
		}
		...
		if (dai_link[i].codecs->dai_name && !dai_link[i].codecs->of_node) {
			index = of_property_match_string(cdev->of_node,
						 "asoc-codec-names",
			...
			if (!np) {
				pr_err("%s: retrieving phandle for codec %s failed\n",
					__func__, dai_link[i].codecs->dai_name);
				ret = -ENODEV;
				goto err;
			}
			dai_link[i].codecs->of_node = np;
			dai_link[i].codecs->dai_name = NULL;
		}
	}
				...
				struct snd_soc_card *card)
{
	...
	for (i = 0; i < card->num_aux_devs; i++) {
		...
		mdm_aux_dev[i].dlc.dai_name = NULL;
