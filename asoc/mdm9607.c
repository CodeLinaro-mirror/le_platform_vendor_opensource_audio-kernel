...
#include "codecs/wcd9330/wcd9330.h"
...
struct mdm9607_codec {
	int (*mclk_enable_fn)(struct snd_soc_component *component,
			      int mclk_enable, bool dapm);
	int (*mbhc_hs_detect)(struct snd_soc_component *component,
...
static const char *const tdm_ch_text[] = {"One", "Two", "Three", "Four",
	"Five", "Six", "Seven", "Eight"};
static const char *const tdm_bit_format_text[] = {"S16_LE", "S24_LE", "S24_3LE",
					    "S32_LE"};
static const char *const tdm_sample_rate_text[] = {"KHZ_16", "KHZ_48"};

static const char *const tdm_num_slots_text[] = {"eight", "sixteen"};
...
#endif
static const struct snd_soc_ops mdm_mi2s_be_ops = {
	.startup = mdm_mi2s_startup,
	.shutdown = mdm_mi2s_shutdown,
};
#ifdef mdm9607daisupport
static const struct snd_soc_ops mdm_sec_mi2s_be_ops = {
...
static int mdm_sec_mi2s_rate_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: mdm_sec_mi2s_rate  = %d\n", __func__,
		 mdm_sec_mi2s_rate);
...
static int mdm_mi2s_get_spk(struct snd_kcontrol *kcontrol,
		       struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s mdm_spk_control %d\n", __func__, mdm_spk_control);
...
static const struct snd_soc_ops mdm_auxpcm_be_ops = {
	.startup = mdm_auxpcm_startup,
	.shutdown = mdm_auxpcm_shutdown,
};

static const struct snd_soc_ops mdm_sec_auxpcm_be_ops = {
...
static const struct snd_soc_ops mdm_tdm_be_ops = {
...
static int mdm_mi2s_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (!strcmp(card->name, "mdm9607-tomtom-i2s-snd-card")) {
		...
		ret = mdm_enable_codec_ext_clk(component, 1, false);
		if (IS_ERR_VALUE(ret)) {
		...
		ret = mdm_enable_codec_ext_clk(component, 0, false);
		if (IS_ERR_VALUE(ret)) {
...
static struct snd_soc_dai_link mdm_dai[] = {
	...
	},
	{
		...
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
		SND_SOC_DAILINK_REG(csvoicehostrxplayback),
	},
	{
		...
		.trigger = {SND_SOC_DPCM_TRIGGER_POST,
			    SND_SOC_DPCM_TRIGGER_POST},
		SND_SOC_DAILINK_REG(csvoicehosttxcapture),
	},
	{
		...
		.ignore_suspend = 1,
		 .ignore_pmdown_time = 1,
		 SND_SOC_DAILINK_REG(csvoicehosttxplayback),
	},
	{
		.name = "MDM Media2",
...
static struct snd_soc_dai_link mdm_9330_dai[] = {
		/* Backend DAI Links */
	{
		...
		.ignore_pmdown_time = 1,
		.ignore_suspend = 1,
		SND_SOC_DAILINK_REG(tomtom_i2s_rx1),
	},
	{
