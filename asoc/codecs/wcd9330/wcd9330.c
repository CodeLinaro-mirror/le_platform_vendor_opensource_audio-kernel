...
int tomtom_enable_qfuse_sensing(struct snd_soc_component *component)
{
	...
	if ((snd_soc_component_read32(component, TOMTOM_A_QFUSE_STATUS) & (0x03)) != 0x03)
...
{
	if (path == RX8_PATH)
		return snd_soc_component_read32(component, TOMTOM_A_CDC_RX8_B5_CTL);
	else
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		(snd_soc_component_read32(component, (TOMTOM_A_CDC_IIR1_CTL + 16 * iir_idx)) &
		(1 << band_idx)) != 0;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	tomtom_mad_input = snd_soc_component_read32(component, TOMTOM_A_CDC_MAD_INP_SEL);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	ear_pa_gain = snd_soc_component_read32(component, TOMTOM_A_RX_EAR_GAIN);
				  ...
				  struct snd_kcontrol *kcontrol, int event)
{
	...
	mad_micb = snd_soc_component_read32(component, TOMTOM_A_MAD_ANA_CTRL) & 0x07;
	switch (mad_micb) {
	case 1:
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		break;
	case SND_SOC_DAPM_POST_PMU:
		if ((snd_soc_component_read32(component, TOMTOM_A_RX_HPH_L_DAC_CTL) & 0x80) &&
			(snd_soc_component_read32(component, TOMTOM_A_RX_HPH_R_DAC_CTL)
								& 0x80)) {
			snd_soc_component_update_bits(component,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		break;
	case SND_SOC_DAPM_POST_PMD:
		if ((snd_soc_component_read32(component, w->reg) & 0x03) == 0)
			snd_soc_component_update_bits(component, WCD9XXX_A_CDC_CLK_OTHR_CTL,
							0x80, 0x00);
		break;
	}
	...
	u8 rx_fs_rate_reg_val, u32 compander_fs, u32 sample_rate)
{
	...
	list_for_each_entry(ch, &tomtom->dai[dai->id].wcd9xxx_ch_list, list) {
		...
		for (j = 0; j < NUM_INTERPOLATORS - 1; j++) {
			...
			rx_mix_1_reg_1_val = snd_soc_component_read32(component,
							  rx_mix_1_reg_1);
			rx_mix_1_reg_2_val = snd_soc_component_read32(component,
							  rx_mix_1_reg_2);
	...
	u8 tx_fs_rate_reg_val, u32 sample_rate)
{
	...
	list_for_each_entry(ch, &tomtom->dai[dai->id].wcd9xxx_ch_list, list) {
		...
		tx_port_reg_val =  snd_soc_component_read32(component, tx_port_reg);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		reg_val = snd_soc_component_read32(component, TOMTOM_A_CDC_CONN_CLSH_CTL);
...
/* 	for (i = 0; i < wcd9xxx->num_of_supplies; i++) {
		if (pdata->regulator[i].name &&
			...
			if (pdata->regulator[i].min_uV == 1800000 &&
				...
				snd_soc_component_write(component, TOMTOM_A_BIAS_REF_CTL,
					      0x1C);
...
{
	struct snd_soc_component *component = mbhc->component;
	const int wg_time = snd_soc_component_read32(component, WCD9XXX_A_RX_HPH_CNP_WG_TIME) *
	...
	case MBHC_ZDET_PA_DISABLE:
		...
		if (!mbhc->hph_pa_dac_state &&
			...
			wcd9xxx_enable_static_pa(mbhc, HPH_PA_DISABLE,
						 HPH_PA_L_R);
		else if (!(snd_soc_component_read32(component, WCD9XXX_A_RX_HPH_CNP_EN) & 0x10))
				     ...
				     uint32_t *zr)
{
	...
	if ((tomtom->zdet_gain_mul_fact == TOMTOM_ZDET_MUL_FACTOR_1X) ||
	    (tomtom->zdet_gain_mul_fact == TOMTOM_ZDET_MUL_FACTOR_10X)) {
		q1_t = ((snd_soc_component_read32(component, TOMTOM_A_QFUSE_DATA_OUT0) &
			0x3) << 0x5);
		q1_t |= ((snd_soc_component_read32(component, TOMTOM_A_QFUSE_DATA_OUT1) &
			0xF8) >> 0x3);
		q2_t = ((snd_soc_component_read32(component, TOMTOM_A_QFUSE_DATA_OUT1) &
			0x7) << 0x4);
		q2_t |= ((snd_soc_component_read32(component, TOMTOM_A_QFUSE_DATA_OUT2) &
			0xF0) >> 0x4);
		...
		q1_t = ((snd_soc_component_read32(component, TOMTOM_A_QFUSE_DATA_OUT2) &
			0xF) << 0x2);
		q1_t |= ((snd_soc_component_read32(component, TOMTOM_A_QFUSE_DATA_OUT3) &
			0xC0) >> 0x6);
		q2_t = (snd_soc_component_read32(component, TOMTOM_A_QFUSE_DATA_OUT3) & 0x3F);
		/* Take out the numeric part of the Qfuse value */
		q1_m = q1_t & 0x1F;
		q2_m = q2_t & 0x1F;
		/* Check the sign part of the Qfuse and adjust value */
...
{
	return !(snd_soc_component_read32(component, WCD9XXX_A_MBHC_INSERT_DET_STATUS) &
			    (1 << 4));
}
