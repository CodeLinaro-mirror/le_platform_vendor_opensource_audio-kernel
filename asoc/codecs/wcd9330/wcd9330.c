...
#include "wcd9330.h"
#include <asoc/wcd9xxx-resmgr.h>
#include <asoc/wcd9xxx-common.h>
...
struct tomtom_priv {
	...
	int (*codec_ext_clk_en_cb)(struct snd_soc_component *component,
...
int tomtom_enable_qfuse_sensing(struct snd_soc_component *component)
{
	struct tomtom_priv *tomtom = snd_soc_component_get_drvdata(component);

	if (tomtom->wcd_ext_clk)
		tomtom_codec_mclk_enable(component, true, false);

	snd_soc_component_write(component, TOMTOM_A_QFUSE_CTL, 0x03);
	/*
	 * 5ms sleep required after enabling qfuse control
	 * before checking the status.
	 */
	usleep_range(5000, 5500);
	if ((snd_soc_component_read(component, TOMTOM_A_QFUSE_STATUS) & (0x03)) != 0x03)
		WARN(1, "%s: Qfuse sense is not complete\n", __func__);

	if (tomtom->wcd_ext_clk)
		tomtom_codec_mclk_enable(component, false, false);
	return 0;
}
EXPORT_SYMBOL_GPL(tomtom_enable_qfuse_sensing);

static int tomtom_get_sample_rate(struct snd_soc_component *component, int path)
{
	if (path == RX8_PATH)
		return snd_soc_component_read(component, TOMTOM_A_CDC_RX8_B5_CTL);
	else
		return snd_soc_component_read(component,
			(TOMTOM_A_CDC_RX1_B5_CTL + 8 * (path - 1)));
}

static int tomtom_compare_bit_format(struct snd_soc_component *component,
				int bit_format)
{
	int i = 0;
	int ret = 0;
	struct tomtom_priv *tomtom_p = snd_soc_component_get_drvdata(component);
...
static int tomtom_update_uhqa_mode(struct snd_soc_component *component, int path)
{
	struct tomtom_priv *tomtom_p = snd_soc_component_get_drvdata(component);

	/* UHQA path has fs=192KHz & bit=24 bit */
	if (((tomtom_get_sample_rate(component, path) & 0xE0) == 0xA0) &&
		(tomtom_compare_bit_format(component, 24))) {
		tomtom_p->uhqa_mode = 1;
	} else {
		tomtom_p->uhqa_mode = 0;
	}
	dev_dbg(component->dev, "%s: uhqa_mode=%d", __func__, tomtom_p->uhqa_mode);
	return 0;
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = (tomtom->anc_func ? 1 : 0);
	return 0;
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_dbg(component->dev, "%s: anc_func %x", __func__, tomtom->anc_func);
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		(snd_soc_component_read(component, (TOMTOM_A_CDC_IIR1_CTL + 16 * iir_idx)) &
		(1 << band_idx)) != 0;
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	snd_soc_component_update_bits(component, (TOMTOM_A_CDC_IIR1_CTL + 16 * iir_idx),
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (comp == COMPANDER_1 &&
		...
		/* Wavegen to 5 msec */
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_CNP_WG_CTL, 0xDB);
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_CNP_WG_TIME, 0x2A);
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_BIAS_WG_OCP, 0x2A);

		/* Enable Chopper */
		snd_soc_component_update_bits(component,
			TOMTOM_A_RX_HPH_CHOP_CTL, 0x80, 0x80);

		snd_soc_component_write(component, TOMTOM_A_NCP_DTEST, 0x20);
		...
		/* Wavegen to 20 msec */
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_CNP_WG_CTL, 0xDB);
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_CNP_WG_TIME, 0x58);
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_BIAS_WG_OCP, 0x1A);

		/* Disable CHOPPER block */
		snd_soc_component_update_bits(component,
			TOMTOM_A_RX_HPH_CHOP_CTL, 0x80, 0x00);

		snd_soc_component_write(component, TOMTOM_A_NCP_DTEST, 0x10);
...
static int tomtom_config_gain_compander(struct snd_soc_component *component,
				       int comp, bool enable)
{
	...
	switch (comp) {
	case COMPANDER_0:
		snd_soc_component_update_bits(component, TOMTOM_A_SPKR_DRV1_GAIN,
				    1 << 2, !enable << 2);
		snd_soc_component_update_bits(component, TOMTOM_A_SPKR_DRV2_GAIN,
				    1 << 2, !enable << 2);
		break;
	case COMPANDER_1:
		snd_soc_component_update_bits(component, TOMTOM_A_RX_HPH_L_GAIN,
				    1 << 5, !enable << 5);
		snd_soc_component_update_bits(component, TOMTOM_A_RX_HPH_R_GAIN,
				    1 << 5, !enable << 5);
		break;
	case COMPANDER_2:
		snd_soc_component_update_bits(component, TOMTOM_A_RX_LINE_1_GAIN,
				    1 << 5, !enable << 5);
		snd_soc_component_update_bits(component, TOMTOM_A_RX_LINE_3_GAIN,
				    1 << 5, !enable << 5);
		snd_soc_component_update_bits(component, TOMTOM_A_RX_LINE_2_GAIN,
				    1 << 5, !enable << 5);
		snd_soc_component_update_bits(component, TOMTOM_A_RX_LINE_4_GAIN,
				    1 << 5, !enable << 5);
		break;
	default:
		WARN_ON(1);
...
static void tomtom_discharge_comp(struct snd_soc_component *component, int comp)
{
	/* Level meter DIV Factor to 5*/
	snd_soc_component_update_bits(component, TOMTOM_A_CDC_COMP0_B2_CTL + (comp * 8), 0xF0,
			    0x05 << 4);
	/* RMS meter Sampling to 0x01 */
	snd_soc_component_write(component, TOMTOM_A_CDC_COMP0_B3_CTL + (comp * 8), 0x01);
				  ...
				  struct snd_kcontrol *kcontrol, int event)
{
	...
	pr_debug("%s: %s event %d compander %d, enabled %d\n", __func__,
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_CLK_RX_B2_CTL,
				    mask << comp_shift[comp],
				    mask << comp_shift[comp]);
		/* Toggle compander reset bits */
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_CLK_OTHR_RESET_B2_CTL,
				    mask << comp_shift[comp],
				    mask << comp_shift[comp]);
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_CLK_OTHR_RESET_B2_CTL,
		...
		tomtom_config_gain_compander(component, comp, true);

		/* Compander enable */
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_COMP0_B1_CTL +
				    (comp * 8), enable_mask, enable_mask);

		tomtom_discharge_comp(component, comp);

		/* Set sample rate dependent parameter */
		snd_soc_component_write(component, TOMTOM_A_CDC_COMP0_B3_CTL + (comp * 8),
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		tomtom_config_gain_compander(component, comp, false);
		break;
	}
	return 0;
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	tomtom_mad_input = snd_soc_component_read(component, TOMTOM_A_CDC_MAD_INP_SEL);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (mic_bias_found) {
		...
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_MAD_INP_SEL,
					0x0F, tomtom_mad_input);
		snd_soc_component_update_bits(component, TOMTOM_A_MAD_ANA_CTRL,
					0x07, mic_bias_found);
		...
		if (strnstr(source_widget, "Internal1", strlen(source_widget)))
			 ...
			 */
			snd_soc_component_write(component, micb_int_reg, 0x24);
		return 0;
	}
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	ear_pa_gain = snd_soc_component_read(component, TOMTOM_A_RX_EAR_GAIN);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	snd_soc_component_update_bits(component, TOMTOM_A_RX_EAR_GAIN, 0xE0, ear_pa_gain);
	return 0;
}
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	widget_name = kstrndup(widget->name, 15, GFP_KERNEL);
	if (!widget_name)
		return -ENOMEM;
	temp = widget_name;
	...
	widget_name = temp;
	if (!dec_name) {
		pr_err("%s: Invalid decimator\n", __func__);
		ret =  -EINVAL;
		goto out;
	}
	dec = strpbrk(dec_name, "123456789");
	if (!dec) {
	/* dev_err("%s: decimator index not found\n",
	 *  __func__);
	 */
		ret =  -EINVAL;
		goto out;
...
static void tomtom_codec_enable_adc_block(struct snd_soc_component *component,
					 int enable)
{
	struct tomtom_priv *tomtom = snd_soc_component_get_drvdata(component);
	...
	if (enable) {
		tomtom->adc_count++;
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_CLK_OTHR_CTL,
						0x2, 0x2);
	} else {
		tomtom->adc_count--;
		if (!tomtom->adc_count)
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_component_update_bits(component, tx_fe_clkdiv_reg, tx_fe_clkdiv_mask,
				    0x0);
		set_bit(bit_pos, &priv->status_mask);
		tomtom_codec_enable_adc_block(component, 1);
	...
	case SND_SOC_DAPM_POST_PMD:
		tomtom_codec_enable_adc_block(component, 0);
		break;
	}
	return 0;
}

static int tomtom_codec_ext_clk_en(struct snd_soc_component *component,
		int enable, bool dapm)
{
	struct tomtom_priv *tomtom = snd_soc_component_get_drvdata(component);
	...
	return tomtom->codec_ext_clk_en_cb(component, enable, dapm);
}
...
int tomtom_codec_mclk_enable(struct snd_soc_component *component,
			     int enable, bool dapm)
{
	struct tomtom_priv *tomtom = snd_soc_component_get_drvdata(component);
	...
	if (tomtom->wcd_ext_clk) {
		...
		return tomtom_codec_ext_clk_en(component, enable, dapm);
	}
	dev_err(component->dev,
...
EXPORT_SYMBOL_GPL(tomtom_codec_mclk_enable);
 ...
 */
static int tomtom_codec_internal_rco_ctrl(struct snd_soc_component *component,
					  bool enable)
{
	struct tomtom_priv *tomtom = snd_soc_component_get_drvdata(component);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		tomtom_codec_internal_rco_ctrl(component, true);
		WCD9XXX_BG_CLK_LOCK(&tomtom->resmgr);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		WCD9XXX_BG_CLK_UNLOCK(&tomtom->resmgr);
		tomtom_codec_internal_rco_ctrl(component, false);
		WCD9XXX_BG_CLK_LOCK(&tomtom->resmgr);
		wcd9xxx_resmgr_put_bandgap(&tomtom->resmgr,
					   WCD9XXX_BANDGAP_AUDIO_MODE);
		WCD9XXX_BG_CLK_UNLOCK(&tomtom->resmgr);
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_component_update_bits(component, lineout_gain_reg, 0x40, 0x40);
		break;
	case SND_SOC_DAPM_POST_PMU:
		wcd9xxx_clsh_fsm(component, &tomtom->clsh_d,
		...
		break;
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(component, lineout_gain_reg, 0x40, 0x00);
				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(component, spk_drv_reg, 0x80, 0x80);
		break;
	case SND_SOC_DAPM_POST_PMD:
		tomtom->spkr_pa_widget_on = false;
		snd_soc_component_update_bits(component, spk_drv_reg, 0x80, 0x00);
		break;
	}
	return 0;
}

static u8 tomtom_get_dmic_clk_val(struct snd_soc_component *component,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		dmic_rate_val =
			tomtom_get_dmic_clk_val(component,
					pdata->mclk_rate,
		...
		if (*dmic_clk_cnt == 1) {
			...
			snd_soc_component_update_bits(component, TOMTOM_A_DMIC_B1_CTL,
...
static int tomtom_codec_config_mad(struct snd_soc_component *component)
{
	int ret = 0;
	...
	if (!tomtom->fw_data) {
		dev_err(component->dev, "%s: invalid cal data\n",
				__func__);
		return -ENODEV;
	}
	...
	if (hwdep_cal) {
		...
		if (ret != 0) {
			pr_err("Failed to acquire MAD firmware data %s: %d\n",
				filename, ret);
			return -ENODEV;
		}
		if (!fw) {
			dev_err(component->dev, "failed to get mad fw\n");
			return -ENODEV;
		}
		data = fw->data;
		cal_size = fw->size;
	...
	snd_soc_component_write(component, TOMTOM_A_CDC_MAD_MAIN_CTL_2,
		      mad_cal->microphone_info.cycle_time);
	snd_soc_component_update_bits(component, TOMTOM_A_CDC_MAD_MAIN_CTL_1, 0xFF << 3,
			    ((uint16_t)mad_cal->microphone_info.settle_time)
			    << 3);

	/* Audio */
	snd_soc_component_write(component, TOMTOM_A_CDC_MAD_AUDIO_CTL_8,
		      mad_cal->audio_info.rms_omit_samples);
	snd_soc_component_update_bits(component, TOMTOM_A_CDC_MAD_AUDIO_CTL_1,
			    0x07 << 4, mad_cal->audio_info.rms_comp_time << 4);
	snd_soc_component_update_bits(component, TOMTOM_A_CDC_MAD_AUDIO_CTL_2, 0x03 << 2,
			    mad_cal->audio_info.detection_mechanism << 2);
	snd_soc_component_write(component, TOMTOM_A_CDC_MAD_AUDIO_CTL_7,
		      mad_cal->audio_info.rms_diff_threshold & 0x3F);
	snd_soc_component_write(component, TOMTOM_A_CDC_MAD_AUDIO_CTL_5,
		      mad_cal->audio_info.rms_threshold_lsb);
	snd_soc_component_write(component, TOMTOM_A_CDC_MAD_AUDIO_CTL_6,
		      mad_cal->audio_info.rms_threshold_msb);

	for (idx = 0; idx < ARRAY_SIZE(mad_cal->audio_info.iir_coefficients);
	     idx++) {
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_MAD_AUDIO_IIR_CTL_PTR,
				    0x3F, idx);
		snd_soc_component_write(component, TOMTOM_A_CDC_MAD_AUDIO_IIR_CTL_VAL,
	...
	/* Beacon */
	snd_soc_component_write(component, TOMTOM_A_CDC_MAD_BEACON_CTL_8,
		      mad_cal->beacon_info.rms_omit_samples);
	snd_soc_component_update_bits(component, TOMTOM_A_CDC_MAD_BEACON_CTL_1,
			    0x07 << 4, mad_cal->beacon_info.rms_comp_time);
	snd_soc_component_update_bits(component, TOMTOM_A_CDC_MAD_BEACON_CTL_2, 0x03 << 2,
			    mad_cal->beacon_info.detection_mechanism << 2);
	snd_soc_component_write(component, TOMTOM_A_CDC_MAD_BEACON_CTL_7,
		      mad_cal->beacon_info.rms_diff_threshold & 0x1F);
	snd_soc_component_write(component, TOMTOM_A_CDC_MAD_BEACON_CTL_5,
		      mad_cal->beacon_info.rms_threshold_lsb);
	snd_soc_component_write(component, TOMTOM_A_CDC_MAD_BEACON_CTL_6,
		      mad_cal->beacon_info.rms_threshold_msb);

	/* Ultrasound */
	snd_soc_component_update_bits(component, TOMTOM_A_CDC_MAD_BEACON_CTL_1,
			    0x07 << 4, mad_cal->beacon_info.rms_comp_time);
	snd_soc_component_update_bits(component, TOMTOM_A_CDC_MAD_ULTR_CTL_2, 0x03 << 2,
			    mad_cal->ultrasound_info.detection_mechanism);
	snd_soc_component_write(component, TOMTOM_A_CDC_MAD_ULTR_CTL_7,
		      mad_cal->ultrasound_info.rms_diff_threshold & 0x1F);
	snd_soc_component_write(component, TOMTOM_A_CDC_MAD_ULTR_CTL_5,
		      mad_cal->ultrasound_info.rms_threshold_lsb);
	snd_soc_component_write(component, TOMTOM_A_CDC_MAD_ULTR_CTL_6,
		      mad_cal->ultrasound_info.rms_threshold_msb);

	/* Set MAD intr time to 20 msec */
	snd_soc_component_update_bits(component, 0x4E, 0x01F, 0x13);
				  ...
				  struct snd_kcontrol *kcontrol, int event)
{
	...
	mad_micb = snd_soc_component_read(component, TOMTOM_A_MAD_ANA_CTRL) & 0x07;
	switch (mad_micb) {
	case 1:
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* Undo reset for MAD */
		snd_soc_component_update_bits(component, TOMTOM_A_SVASS_CLKRST_CTL,
				    0x02, 0x00);

		ret = tomtom_codec_config_mad(component);
		if (ret) {
		...
		snd_soc_component_update_bits(component, mad_cfilt_reg,
				    0x02, 0x02);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* Reset the MAD block */
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (strnstr(w->name, internal1_text, 30))
			snd_soc_component_update_bits(component, micb_int_reg, 0xE0, 0xE0);
		else if (strnstr(w->name, internal2_text, 30))
			snd_soc_component_update_bits(component, micb_int_reg, 0x1C, 0x1C);
		else if (strnstr(w->name, internal3_text, 30))
			snd_soc_component_update_bits(component, micb_int_reg, 0x3, 0x3);
		else
			/*
			 *  If not internal, make sure to write the
			 *  register to default value
		...
		if (tomtom->mbhc_started && micb_ctl_reg ==
			...
			if (++tomtom->micb_2_users == 1) {
				if (tomtom->resmgr.pdata->micbias.bias2_is_headset_only)
					...
					snd_soc_component_update_bits(component, micb_ctl_reg,
							    1 << w->shift,
							    1 << w->shift);
			}
			...
			snd_soc_component_update_bits(component, micb_ctl_reg, 1 << w->shift,
					    1 << w->shift);
		}
		break;
	case SND_SOC_DAPM_POST_PMU:
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (strnstr(w->name, internal1_text, 30))
			snd_soc_component_update_bits(component, micb_int_reg, 0x80, 0x00);
		else if (strnstr(w->name, internal2_text, 30))
			snd_soc_component_update_bits(component, micb_int_reg, 0x10, 0x00);
		else if (strnstr(w->name, internal3_text, 30))
...
static int tomtom_enable_mbhc_micbias(struct snd_soc_component *component, bool enable,
				enum wcd9xxx_micbias_num micb_num)
{
	int rc;

	if (micb_num != MBHC_MICBIAS2) {
		dev_err(component->dev, "%s: Unsupported micbias, micb_num=%d\n",
			__func__, micb_num);
		return -EINVAL;
	}

	if (enable)
		rc = snd_soc_dapm_force_enable_pin(snd_soc_component_get_dapm(component),
					     DAPM_MICBIAS2_EXTERNAL_STANDALONE);
	else
		rc = snd_soc_dapm_disable_pin(snd_soc_component_get_dapm(component),
					     DAPM_MICBIAS2_EXTERNAL_STANDALONE);
	if (!rc)
...
static void txfe_clkdiv_update(struct snd_soc_component *component)
{
	struct tomtom_priv *priv = snd_soc_component_get_drvdata(component);

	if (test_bit(ADC1_TXFE, &priv->status_mask)) {
		snd_soc_component_update_bits(component, TOMTOM_A_TX_1_2_TXFE_CLKDIV,
				    0x0F, 0x05);
	...
	if (test_bit(ADC3_TXFE, &priv->status_mask)) {
		snd_soc_component_update_bits(component, TOMTOM_A_TX_3_4_TXFE_CKDIV,
				    0x0F, 0x05);
	...
	if (test_bit(ADC5_TXFE, &priv->status_mask)) {
		snd_soc_component_update_bits(component, TOMTOM_A_TX_5_6_TXFE_CKDIV,
				    0x0F, 0x05);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (!tx_hpf_work[decimator - 1].tx_hpf_bypass) {
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if ((tx_hpf_work[decimator - 1].tx_hpf_cut_of_freq !=
				CF_MIN_3DB_150HZ) &&
			(!tx_hpf_work[decimator - 1].tx_hpf_bypass)) {
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_CLK_RX_RESET_CTL,
			1 << w->shift, 1 << w->shift);
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_CLK_RX_RESET_CTL,
			1 << w->shift, 0x0);
		break;
	case SND_SOC_DAPM_POST_PMU:
		...
		if (w->shift == 0 || w->shift == 1)
			tomtom_update_uhqa_mode(component, (1 << w->shift));
				      ...
				      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (++priv->ldo_h_users == 1) {
			...
			tomtom_codec_internal_rco_ctrl(component, true);
			snd_soc_component_update_bits(component, TOMTOM_A_LDO_H_MODE_1,
					    1 << 7, 1 << 7);
			tomtom_codec_internal_rco_ctrl(component, false);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (hwdep_cal) {
			...
			ret = request_firmware(&fw, filename, component->dev);
			if (ret != 0) {
				dev_err(component->dev, "Failed to acquire ANC data: %d\n",
					ret);
				return -ENODEV;
			}
			if (!fw) {
		...
		for (i = 0; i < num_anc_slots; i++) {
			if (anc_size_remaining < TOMTOM_PACKED_REG_SIZE) {
				dev_err(component->dev, "Invalid register format\n");
				ret = -EINVAL;
				goto err;
			}
	...
	case SND_SOC_DAPM_PRE_PMD:
		msleep(40);
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_ANC1_B1_CTL, 0x01,
				    0x00);
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_ANC2_B1_CTL, 0x02,
				    0x00);
		msleep(20);
		snd_soc_component_write(component, TOMTOM_A_CDC_CLK_ANC_RESET_CTL, 0x0F);
		snd_soc_component_write(component, TOMTOM_A_CDC_CLK_ANC_CLK_EN_CTL, 0);
		snd_soc_component_write(component, TOMTOM_A_CDC_CLK_ANC_RESET_CTL, 0x00);
		break;
	}
	return 0;
err:
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (!high_perf_mode && !tomtom_p->uhqa_mode) {
			wcd9xxx_clsh_fsm(component, &tomtom_p->clsh_d,
						 WCD9XXX_CLSH_STATE_HPHL,
						 WCD9XXX_CLSH_REQ_ENABLE,
						 WCD9XXX_CLSH_EVENT_PRE_DAC);
		} else {
			wcd9xxx_enable_high_perf_mode(component, &tomtom_p->clsh_d,
						tomtom_p->uhqa_mode,
		...
		ret = wcd9xxx_mbhc_get_impedance(&tomtom_p->mbhc,
					&impedl, &impedr);
		if (!ret)
			wcd9xxx_clsh_imped_config(component, impedl);
		else
			dev_dbg(component->dev, "%s: Failed to get mbhc impedance %d\n",
						__func__, ret);
		break;
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_RX1_B3_CTL, 0xBC, 0x94);
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_RX1_B4_CTL, 0x30, 0x10);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_RX1_B3_CTL, 0xBC, 0x00);
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_RX1_B4_CTL, 0x30, 0x00);
		break;
	case SND_SOC_DAPM_POST_PMD:
		if (!high_perf_mode && !tomtom_p->uhqa_mode) {
			wcd9xxx_clsh_fsm(component, &tomtom_p->clsh_d,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		break;
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_RX2_B3_CTL, 0xBC, 0x94);
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_RX2_B4_CTL, 0x30, 0x10);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_RX2_B3_CTL, 0xBC, 0x00);
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_RX2_B4_CTL, 0x30, 0x00);
		break;
	case SND_SOC_DAPM_POST_PMD:
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		break;
	case SND_SOC_DAPM_POST_PMU:
		if ((snd_soc_component_read(component, TOMTOM_A_RX_HPH_L_DAC_CTL) & 0x80) &&
			(snd_soc_component_read(component, TOMTOM_A_RX_HPH_R_DAC_CTL)
								& 0x80)) {
			snd_soc_component_update_bits(component,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		break;
	case SND_SOC_DAPM_POST_PMD:
		if ((snd_soc_component_read(component, w->reg) & 0x03) == 0)
			snd_soc_component_update_bits(component, WCD9XXX_A_CDC_CLK_OTHR_CTL,
							0x80, 0x00);
		break;
	}
...
int tomtom_mclk_enable(struct snd_soc_component *component, int mclk_enable, bool dapm)
{
	struct tomtom_priv *tomtom = snd_soc_component_get_drvdata(component);
...
{
	u8 val = 0;
	struct tomtom_priv *tomtom = snd_soc_component_get_drvdata(dai->component);
...

{
	...
	struct tomtom_priv *tomtom = snd_soc_component_get_drvdata(dai->component);
	struct wcd9xxx *core = dev_get_drvdata(dai->component->dev->parent);
...
{
	struct tomtom_priv *tomtom_p = snd_soc_component_get_drvdata(dai->component);
	u32 i = 0;
	...
	u8 rx_fs_rate_reg_val, u32 compander_fs, u32 sample_rate)
{
	...
	list_for_each_entry(ch, &tomtom->dai[dai->id].wcd9xxx_ch_list, list) {
		...
		for (j = 0; j < NUM_INTERPOLATORS - 1; j++) {
			...
			rx_mix_1_reg_1_val = snd_soc_component_read(component,
							  rx_mix_1_reg_1);
			rx_mix_1_reg_2_val = snd_soc_component_read(component,
							  rx_mix_1_reg_2);
			...
			if (((rx_mix_1_reg_1_val & 0x0F) == rx_mix1_inp) ||
				...
				snd_soc_component_update_bits(component, rx_fs_reg,
		...
		if (((rx_mix_1_reg_1_val & 0x0F) == rx8_mix1_inp) ||
			...
			snd_soc_component_update_bits(component, TOMTOM_A_CDC_RX8_B5_CTL,
					    0xE0, rx_fs_rate_reg_val);
			pr_debug("%s: AIF_PB DAI(%d) connected to RX%u\n",
	...
	u8 tx_fs_rate_reg_val, u32 sample_rate)
{
	...
	list_for_each_entry(ch, &tomtom->dai[dai->id].wcd9xxx_ch_list, list) {
		...
		tx_port_reg_val =  snd_soc_component_read(component, tx_port_reg);
		...
		if (decimator) { /* SLIM_TX port has a DEC as input */
			...
			snd_soc_component_update_bits(component, tx_fs_reg, 0x07,
					    tx_fs_rate_reg_val);
				       ...
				       struct snd_soc_dai *dai)
{
	...
	case 24:
		...
		break;
	default:
		dev_err(component->dev, "Invalid format\n");
		return;
	}
	...
	list_for_each_entry(ch, &cdc_dai->wcd9xxx_ch_list, list) {
		...
		dev_dbg(component->dev, "%s: sb_ctl_reg %x field_shift %x\n",
			__func__, sb_ctl_reg, field_shift);
		snd_soc_component_update_bits(component, sb_ctl_reg, 0x3 << field_shift,
				    bit_sel << field_shift);
	}
}
					 ...
					 struct snd_soc_dai *dai)
{
	...
	case 24:
		...
		break;
	default:
		dev_err(component->dev, "%s: Invalid format %d\n", __func__,
			params_width(params));
		return;
	}
	...
	list_for_each_entry(ch, &cdc_dai->wcd9xxx_ch_list, list) {
		...
		dev_dbg(component->dev, "%s: reg %x bit_sel %x bit_shift %x\n",
			__func__, sb_ctl_reg, bit_sel, bit_shift);
		snd_soc_component_update_bits(component, sb_ctl_reg, 0x3 <<
			    ...
			    struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_STREAM_CAPTURE:
		...
		if (tomtom->intf_type == WCD9XXX_INTERFACE_TYPE_I2C) {
			snd_soc_component_update_bits(component, TOMTOM_A_CDC_CLK_TX_I2S_CTL,
					    0x20, i2s_bit_mode << 5);
			snd_soc_component_update_bits(component, TOMTOM_A_CDC_CLK_TX_I2S_CTL,
					    0x07, tx_fs_rate);
		} else {
	...
	case SNDRV_PCM_STREAM_PLAYBACK:
		...
		if (tomtom->intf_type == WCD9XXX_INTERFACE_TYPE_I2C) {
			...
			snd_soc_component_update_bits(component, TOMTOM_A_CDC_CLK_RX_I2S_CTL,
...
static const struct snd_soc_dai_ops tomtom_dai_ops = {
				...
				int event)
{
	...
	tomtom_p = snd_soc_component_get_drvdata(component);
	core = dev_get_drvdata(component->dev->parent);
	...
	/* Execute the callback only if interface type is slimbus */
	if (tomtom_p->intf_type != WCD9XXX_INTERFACE_TYPE_SLIMBUS) {
		pr_err("%s Interface is not correct\n", __func__);
		return 0;
	}
	...
	case SND_SOC_DAPM_POST_PMU:
		/*Enable V&I sensing*/
		snd_soc_component_update_bits(component, TOMTOM_A_SPKR1_PROT_EN,
				0x88, 0x88);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_CLK_TX_CLK_EN_B2_CTL,
				0xC, 0x0);
		/*Disable V&I sensing*/
		snd_soc_component_update_bits(component, TOMTOM_A_SPKR1_PROT_EN,
				0x88, 0x00);
 ...
 */
static int __tomtom_codec_enable_slimtx(struct snd_soc_component *component,
 ...
 */
static int tomtom_codec_enable_slimtx_mad(struct snd_soc_component *component,
					  u8 event)
{
	struct tomtom_priv *tomtom_p = snd_soc_component_get_drvdata(component);
	struct wcd9xxx_codec_dai_data *dai;
	...
	dev_dbg(component->dev,
		"%s: mad_channel, event = 0x%x\n",
		 __func__, event);
	return __tomtom_codec_enable_slimtx(component, dapm_event, dai);
}
				     ...
				     int event)
{
	...
	dai = &tomtom_p->dai[w->shift];
	return __tomtom_codec_enable_slimtx(component, event, dai);
}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMD:
		if (strnstr(w->name, "IIR1", sizeof("IIR1"))) {
			snd_soc_component_write(component, TOMTOM_A_CDC_IIR1_GAIN_B1_CTL,
				      snd_soc_component_read(component,
					      TOMTOM_A_CDC_IIR1_GAIN_B1_CTL));
			snd_soc_component_write(component, TOMTOM_A_CDC_IIR1_GAIN_B2_CTL,
				      snd_soc_component_read(component,
					      TOMTOM_A_CDC_IIR1_GAIN_B2_CTL));
			snd_soc_component_write(component, TOMTOM_A_CDC_IIR1_GAIN_B3_CTL,
				      snd_soc_component_read(component,
					      TOMTOM_A_CDC_IIR1_GAIN_B3_CTL));
			snd_soc_component_write(component, TOMTOM_A_CDC_IIR1_GAIN_B4_CTL,
				      snd_soc_component_read(component,
					      TOMTOM_A_CDC_IIR1_GAIN_B4_CTL));
		} else {
			snd_soc_component_write(component, TOMTOM_A_CDC_IIR2_GAIN_B1_CTL,
				      snd_soc_component_read(component,
					      TOMTOM_A_CDC_IIR2_GAIN_B1_CTL));
			snd_soc_component_write(component, TOMTOM_A_CDC_IIR2_GAIN_B2_CTL,
				      snd_soc_component_read(component,
					      TOMTOM_A_CDC_IIR2_GAIN_B2_CTL));
			snd_soc_component_write(component, TOMTOM_A_CDC_IIR2_GAIN_B3_CTL,
				      snd_soc_component_read(component,
					      TOMTOM_A_CDC_IIR2_GAIN_B3_CTL));
			snd_soc_component_write(component, TOMTOM_A_CDC_IIR2_GAIN_B4_CTL,
				      snd_soc_component_read(component,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		reg_val = snd_soc_component_read(component, TOMTOM_A_CDC_CONN_CLSH_CTL);
	...
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_component_update_bits(component, TOMTOM_A_CDC_CONN_CLSH_CTL,
							0x0C, 0x00);
		break;
	}
	return 0;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		ret = tomtom_codec_enable_anc(w, kcontrol, event);
		msleep(50);
		snd_soc_component_update_bits(component, TOMTOM_A_RX_EAR_EN, 0x10, 0x10);
		break;
	case SND_SOC_DAPM_POST_PMU:
		ret = tomtom_codec_enable_ear_pa(w, kcontrol, event);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_component_update_bits(component, TOMTOM_A_RX_EAR_EN, 0x10, 0x00);
		msleep(40);
		ret |= tomtom_codec_enable_anc(w, kcontrol, event);
		break;
	case SND_SOC_DAPM_POST_PMD:
	...
	for_each_set_bit(j, &status, 32) {
		...
		if ((val & TOMTOM_SLIM_IRQ_OVERFLOW) ||
			...
			if (!tx)
				...
				reg = TOMTOM_SLIM_PGD_PORT_INT_TX_EN0 +
					(port_id / 8);
			int_val = wcd9xxx_interface_reg_read(priv->wcd9xxx, reg);
			if (int_val & (1 << (port_id % 8))) {
...
static int tomtom_handle_pdata(struct tomtom_priv *tomtom)
{
	...
	snd_soc_component_update_bits(component, TOMTOM_A_LDO_H_MODE_1, 0x0C,
			    (pdata->micbias.ldoh_v << 2));

	snd_soc_component_update_bits(component, TOMTOM_A_MICB_CFILT_1_VAL, 0xFC, (k1 << 2));
	snd_soc_component_update_bits(component, TOMTOM_A_MICB_CFILT_2_VAL, 0xFC, (k2 << 2));
	snd_soc_component_update_bits(component, TOMTOM_A_MICB_CFILT_3_VAL, 0xFC, (k3 << 2));

	snd_soc_component_update_bits(component, TOMTOM_A_MICB_1_CTL, 0x60,
			    (pdata->micbias.bias1_cfilt_sel << 5));
	snd_soc_component_update_bits(component, TOMTOM_A_MICB_2_CTL, 0x60,
			    (pdata->micbias.bias2_cfilt_sel << 5));
	snd_soc_component_update_bits(component, TOMTOM_A_MICB_3_CTL, 0x60,
			    (pdata->micbias.bias3_cfilt_sel << 5));
	snd_soc_component_update_bits(component, tomtom->resmgr.reg_addr->micb_4_ctl, 0x60,
	...
	if (flag & 0x40) {
		...
		snd_soc_component_update_bits(component, TOMTOM_A_TX_7_MBHC_EN,
			0x13, value);
	}
	...
	if (pdata->ocp.use_pdata) {
		...
		snd_soc_component_update_bits(component, TOMTOM_A_RX_COM_OCP_CTL,
			0x0F, pdata->ocp.num_attempts);
		snd_soc_component_write(component, TOMTOM_A_RX_COM_OCP_COUNT,
			((pdata->ocp.run_time << 4) | pdata->ocp.wait_time));
		snd_soc_component_update_bits(component, TOMTOM_A_RX_HPH_OCP_CTL,
	 ...
	 *			snd_soc_component_write(component, TOMTOM_A_BIAS_REF_CTL, 0x1E);
	 *		} else {
	 *			pr_err("%s: unsupported CDC_VDDA_RX voltage\n"
	 *					"min %d, max %d\n",
	 *					__func__, pdata->regulator[i].min_uV,
	...
	value = (pdata->micbias.bias1_cap_mode == MICBIAS_EXT_BYP_CAP ?
		 0x00 : 0x16);
	snd_soc_component_update_bits(component, TOMTOM_A_MICB_1_CTL, 0x1E, value);
	value = (pdata->micbias.bias2_cap_mode == MICBIAS_EXT_BYP_CAP ?
		 0x00 : 0x16);
	snd_soc_component_update_bits(component, TOMTOM_A_MICB_2_CTL, 0x1E, value);
	value = (pdata->micbias.bias3_cap_mode == MICBIAS_EXT_BYP_CAP ?
		 0x00 : 0x16);
	snd_soc_component_update_bits(component, TOMTOM_A_MICB_3_CTL, 0x1E, value);
	value = (pdata->micbias.bias4_cap_mode == MICBIAS_EXT_BYP_CAP ?
		 0x00 : 0x16);
	snd_soc_component_update_bits(component, TOMTOM_A_MICB_4_CTL, 0x1E, value);
	 ...
	 */
	mad_dmic_ctl_val = tomtom_get_dmic_clk_val(tomtom->component,
				pdata->mclk_rate,
				pdata->mad_dmic_sample_rate);
	snd_soc_component_update_bits(component, TOMTOM_A_DMIC_B1_CTL,
		0xE0, mad_dmic_ctl_val << 5);
	snd_soc_component_update_bits(component, TOMTOM_A_DMIC_B2_CTL,
		0x70, mad_dmic_ctl_val << 4);
	snd_soc_component_update_bits(component, TOMTOM_A_DMIC_B2_CTL,
		0x0E, mad_dmic_ctl_val << 1);

	dmic_ctl_val = tomtom_get_dmic_clk_val(tomtom->component,
				pdata->mclk_rate,
				pdata->dmic_sample_rate);
	...
	for (dec = 0; dec < NUM_DECIMATORS; dec++) {
		...
		snd_soc_component_update_bits(component, tx_dmic_ctl_reg,
				    0x07, dmic_ctl_val);
	}
	snd_soc_component_update_bits(component, TOMTOM_A_CDC_ANC1_B2_CTL,
		0x1, anc_ctl_value);
	snd_soc_component_update_bits(component, TOMTOM_A_CDC_ANC2_B2_CTL,
		0x1, anc_ctl_value);
done:
	return rc;
}
...
static void tomtom_update_reg_defaults(struct snd_soc_component *component)
{
	u32 i;
	struct wcd9xxx *tomtom_core = dev_get_drvdata(component->dev->parent);
	struct tomtom_priv *tomtom = snd_soc_component_get_drvdata(component);

	for (i = 0; i < ARRAY_SIZE(tomtom_reg_defaults); i++)
		snd_soc_component_write(component, tomtom_reg_defaults[i].reg,
			      tomtom_reg_defaults[i].val);

	for (i = 0; i < ARRAY_SIZE(tomtom_1_0_reg_defaults); i++)
		snd_soc_component_write(component, tomtom_1_0_reg_defaults[i].reg,
				tomtom_1_0_reg_defaults[i].val);

	if (!TOMTOM_IS_1_0(tomtom_core->version)) {
		for (i = 0; i < ARRAY_SIZE(tomtom_2_0_reg_defaults); i++)
			snd_soc_component_write(component, tomtom_2_0_reg_defaults[i].reg,
...
static void tomtom_codec_init_reg(struct snd_soc_component *component)
{
	u32 i;
	struct wcd9xxx *tomtom_core = dev_get_drvdata(component->dev->parent);

	for (i = 0; i < ARRAY_SIZE(tomtom_codec_reg_init_val); i++)
		snd_soc_component_update_bits(component, tomtom_codec_reg_init_val[i].reg,
...
static void tomtom_slim_interface_init_reg(struct snd_soc_component *component)
{
	int i;
	struct wcd9xxx *wcd9xxx = dev_get_drvdata(component->dev->parent);
...
static int tomtom_setup_irqs(struct tomtom_priv *tomtom)
{
	...
	if (ret)
		...
		tomtom_slim_interface_init_reg(component);
...
static
struct firmware_cal *tomtom_get_hwdep_fw_cal(struct snd_soc_component *component,
...
int tomtom_hs_detect(struct snd_soc_component *component,
		    struct wcd9xxx_mbhc_config *mbhc_cfg)
{
	...
	if (mbhc_cfg->insert_detect) {
		...
		snd_soc_component_update_bits(component, TOMTOM_A_MBHC_INSERT_DETECT2, 0xC0,
				    0x00);
		snd_soc_component_update_bits(component, TOMTOM_A_MICB_CFILT_2_CTL, 0x01,
				    0x00);
...
EXPORT_SYMBOL_GPL(tomtom_hs_detect);

void tomtom_hs_detect_exit(struct snd_soc_component *component)
{
	struct tomtom_priv *tomtom = snd_soc_component_get_drvdata(component);
...
EXPORT_SYMBOL_GPL(tomtom_hs_detect_exit);
...
EXPORT_SYMBOL_GPL(tomtom_event_register);

void tomtom_register_ext_clk_cb(
	int (*codec_ext_clk_en)(struct snd_soc_component *component,
				int enable, bool dapm),
...
EXPORT_SYMBOL_GPL(tomtom_register_ext_clk_cb);

static void tomtom_init_slim_slave_cfg(struct snd_soc_component *component)
{
	struct tomtom_priv *priv = snd_soc_component_get_drvdata(component);
				     ...
				     struct list_head *lh)
{
	...
	for (i = 0; i < ARRAY_SIZE(reg_set_paon); i++) {
		...
		if (reg_set_paon[i].reg == WCD9XXX_A_BUCK_MODE_1 ||
			...
			delay = 0;
		wcd9xxx_soc_update_bits_push(component, lh,
					     reg_set_paon[i].reg,
					     reg_set_paon[i].mask,
...
{
	struct snd_soc_component *component = mbhc->component;
	const int wg_time = snd_soc_component_read(component, WCD9XXX_A_RX_HPH_CNP_WG_TIME) *
			    TOMTOM_WG_TIME_FACTOR_US;
	u8 mask = (hph_pa << 4);
	u8 pa_en = enable ? mask : ~mask;

	snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_CNP_EN, mask, pa_en);
...
#define __wr(reg, mask, value)						  \
	do {								  \
		ret = wcd9xxx_soc_update_bits_push(component,		  \
	...
	case MBHC_ZDET_PRE_MEASURE:
		...
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_EN_CTL, 0x00);
	...
	case MBHC_ZDET_POST_MEASURE:
		...
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B4_CTL, 0x69);
		/* Program the PA Ramp to FS_16K, L shift 1 */
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B3_CTL,
			      0x1 << 4 | 0x6);
		/* Reset the PA Ramp */
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B1_CTL, 0x1C);
		/*
		 * Connect the PA Ramp to PA chain and release reset with
		 * keep it connected.
		 */
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B1_CTL, 0x1F);
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B1_CTL, 0x03);

		/* Start the PA ramp on HPH L and R */
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B2_CTL, 0x05);
	...
	case MBHC_ZDET_GAIN_0:
		/* Set Gain at 1x */
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_L_ATEST, 0x00);
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_R_ATEST, 0x00);
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_L_PA_CTL, 0x42);
	...
	case MBHC_ZDET_GAIN_1:
		/* Set Gain at 10x */
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_L_ATEST, 0x10);
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_R_ATEST, 0x00);
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_L_PA_CTL, 0x42);
		/* Allow 100us for gain registers to settle */
		usleep_range(100,
	...
	case MBHC_ZDET_GAIN_2:
		/* Set Gain at 100x */
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_L_ATEST, 0x00);
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_R_ATEST, 0x10);
		snd_soc_component_write(component, TOMTOM_A_RX_HPH_L_PA_CTL, 0x43);
		/* Allow 100us for gain registers to settle */
		usleep_range(100,
			     100 + WCD9XXX_USLEEP_RANGE_MARGIN_US);
	...
	case MBHC_ZDET_RAMP_DISABLE:
		...
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B2_CTL, 0x00);
		/* 0x69 for 105 number of samples for PA RAMP */
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B4_CTL, 0x69);
		/* Program the PA Ramp to FS_16K, L shift 1 */
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B3_CTL,
			      0x1 << 4 | 0x6);
		/* Reset the PA Ramp */
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B1_CTL, 0x17);
		/*
		 * Connect the PA Ramp to PA chain and release reset with
		 * keep it connected.
		 */
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B1_CTL, 0x03);
		/* Start the PA ramp on HPH L and R */
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B2_CTL, 0x0A);
	...
	case MBHC_ZDET_HPHR_RAMP_DISABLE:
		 ...
		 */
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B1_CTL, 0x03);
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B2_CTL, 0x08);
		/* Ramp generator takes ~30ms to settle down */
		usleep_range(TOMTOM_HPH_PA_RAMP_DELAY,
			     TOMTOM_HPH_PA_RAMP_DELAY +
			     WCD9XXX_USLEEP_RANGE_MARGIN_US);
	...
	case MBHC_ZDET_HPHL_RAMP_DISABLE:
		 ...
		 */
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B1_CTL, 0x03);
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B2_CTL, 0x02);
		/* Ramp generator takes ~30ms to settle down */
		usleep_range(TOMTOM_HPH_PA_RAMP_DELAY,
			     TOMTOM_HPH_PA_RAMP_DELAY +
			     WCD9XXX_USLEEP_RANGE_MARGIN_US);
	...
	case MBHC_ZDET_PA_DISABLE:
		...
		if (!mbhc->hph_pa_dac_state &&
			...
			wcd9xxx_enable_static_pa(mbhc, HPH_PA_DISABLE,
						 HPH_PA_L_R);
		else if (!(snd_soc_component_read(component, WCD9XXX_A_RX_HPH_CNP_EN) & 0x10))
			wcd9xxx_enable_static_pa(mbhc, HPH_PA_ENABLE, HPH_PA_R);

		/* Turn off PA ramp generator */
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B1_CTL, 0x00);
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B2_CTL, 0x00);
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B3_CTL, 0x00);
		snd_soc_component_write(component, WCD9XXX_A_CDC_PA_RAMP_B4_CTL, 0x00);

		/* Restore registers */
		wcd9xxx_restore_registers(component, &tomtom->reg_save_restore);
		break;
	}
#undef __wr
				     ...
				     uint32_t *zl, uint32_t *zr)
{
	...
	if (l && zl) {
		...
		if (zld)
			 ...
			 * Assign a default value so that at least the value
				     ...
				     uint32_t *zr)
{
	...
	if ((tomtom->zdet_gain_mul_fact == TOMTOM_ZDET_MUL_FACTOR_1X) ||
	    (tomtom->zdet_gain_mul_fact == TOMTOM_ZDET_MUL_FACTOR_10X)) {
		q1_t = ((snd_soc_component_read(component, TOMTOM_A_QFUSE_DATA_OUT0) &
			0x3) << 0x5);
		q1_t |= ((snd_soc_component_read(component, TOMTOM_A_QFUSE_DATA_OUT1) &
			0xF8) >> 0x3);
		q2_t = ((snd_soc_component_read(component, TOMTOM_A_QFUSE_DATA_OUT1) &
			0x7) << 0x4);
		q2_t |= ((snd_soc_component_read(component, TOMTOM_A_QFUSE_DATA_OUT2) &
			0xF0) >> 0x4);
		...
		q1_t = ((snd_soc_component_read(component, TOMTOM_A_QFUSE_DATA_OUT2) &
			0xF) << 0x2);
		q1_t |= ((snd_soc_component_read(component, TOMTOM_A_QFUSE_DATA_OUT3) &
			0xC0) >> 0x6);
		q2_t = (snd_soc_component_read(component, TOMTOM_A_QFUSE_DATA_OUT3) & 0x3F);
		/* Take out the numeric part of the Qfuse value */
		q1_m = q1_t & 0x1F;
		q2_m = q2_t & 0x1F;
		/* Check the sign part of the Qfuse and adjust value */
	...
	dev_dbg(component->dev, "%s: qfuse1 = %d, qfuse2 = %d\n",
		__func__, q1, q2);
	if (!q1 && !q2) {
		dev_dbg(component->dev, "%s: qfuse1 and qfuse2 are 0. Exiting\n",
			__func__);
		return;
	}
	...
	dev_dbg(component->dev, "%s: rl = 0x%llx (%lld) \t rr = 0x%llx (%lld)\n",
...
static bool tomtom_mbhc_ins_rem_status(struct snd_soc_component *component)
{
	return !(snd_soc_component_read(component, WCD9XXX_A_MBHC_INSERT_DET_STATUS) &
			    (1 << 4));
}
					   ...
					   bool enable)
{
	...
	if (!enable) {
		...
		snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.cfilt_ctl,
				    0x01, 0x00);
	} else {
...
static void tomtom_codec_hph_auto_pull_down(struct snd_soc_component *component,
					    bool enable)
{
	struct wcd9xxx *tomtom_core = dev_get_drvdata(component->dev->parent);
	...
	dev_dbg(component->dev, "%s: %s auto pull down\n", __func__,
			enable ? "enable" : "disable");
	if (enable) {
		snd_soc_component_update_bits(component, TOMTOM_A_RX_HPH_L_TEST, 0x08, 0x08);
		snd_soc_component_update_bits(component, TOMTOM_A_RX_HPH_R_TEST, 0x08, 0x08);
	} else {
		snd_soc_component_update_bits(component, TOMTOM_A_RX_HPH_L_TEST, 0x08, 0x00);
		snd_soc_component_update_bits(component, TOMTOM_A_RX_HPH_R_TEST, 0x08, 0x00);
	}
}
...
static int tomtom_post_reset_cb(struct wcd9xxx *wcd9xxx)
{
	...
	tomtom_update_reg_defaults(component);
	tomtom->component = component;
	if (wcd9xxx->mclk_rate == TOMTOM_MCLK_CLK_12P288MHZ)
		snd_soc_component_update_bits(component, TOMTOM_A_CHIP_CTL, 0x06, 0x0);
	else if (wcd9xxx->mclk_rate == TOMTOM_MCLK_CLK_9P6MHZ)
		snd_soc_component_update_bits(component, TOMTOM_A_CHIP_CTL, 0x06, 0x2);
	tomtom_codec_init_reg(component);
 ...
 */
	...
	tomtom_init_slim_slave_cfg(component);
	tomtom_slim_interface_init_reg(component);
	...
	if (tomtom->mbhc_started) {
		...
		if (ret)
			...
			tomtom_hs_detect(component, tomtom->mbhc.mbhc_cfg);
	}

	if (tomtom->machine_codec_event_cb)
		tomtom->machine_codec_event_cb(component,
	 ...
	 */
	tomtom_enable_qfuse_sensing(component);
	mutex_unlock(&tomtom->codec_mutex);
	return ret;
}

void *tomtom_get_afe_config(struct snd_soc_component *component,
...
static struct regulator *tomtom_codec_find_regulator(struct snd_soc_component *component,
						    const char *name)
{
	int i;
	struct wcd9xxx *core = dev_get_drvdata(component->dev->parent);
				   ...
				   bool enable)
{
	...
	dev_dbg(component->dev, "%s: %s, mclk_rate = %d\n",
		__func__, (enable ? "enable" : "disable"),
		wcd9xxx->mclk_rate);

	switch (wcd9xxx->mclk_rate) {
	case TOMTOM_MCLK_CLK_9P6MHZ:
		snd_soc_component_update_bits(component, TOMTOM_A_FLL_NREF,
				    0x1F, 0x15);
		snd_soc_component_update_bits(component, TOMTOM_A_FLL_KDCO_TUNE,
				    0x07, 0x06);
		snd_soc_component_write(component, TOMTOM_A_FLL_LOCK_THRESH, 0xD1);
		snd_soc_component_write(component, TOMTOM_A_FLL_LOCK_DET_COUNT,
			      0x40);
		break;
	case TOMTOM_MCLK_CLK_12P288MHZ:
		snd_soc_component_update_bits(component, TOMTOM_A_FLL_NREF,
		...
		snd_soc_component_update_bits(component, TOMTOM_A_FLL_KDCO_TUNE,
				    0x07, 0x05);
		snd_soc_component_write(component, TOMTOM_A_FLL_LOCK_THRESH, 0xB1);
		snd_soc_component_write(component, TOMTOM_A_FLL_LOCK_DET_COUNT,
			      0x40);
		break;
	}
...
static int tomtom_codec_slim_reserve_bw(struct snd_soc_component *component,
...
static int tomtom_codec_vote_max_bw(struct snd_soc_component *component,
			bool vote)
{
	u32 bw_ops;
	...
	return tomtom_codec_slim_reserve_bw(component,
			bw_ops, true);
}
...
static int tomtom_cpe_err_irq_control(struct snd_soc_component *component,
...
static int tomtom_codec_probe(struct snd_soc_component *component)
{
	...
	control->post_reset = tomtom_post_reset_cb;
	...
	if (!of_find_property(wcd9xxx->dev->of_node, "clock-names", NULL)) {
		...
		if (IS_ERR(wcd_ext_clk)) {
			dev_err(component->dev, "%s: clk get %s failed\n",
	...
	ret = wcd9xxx_resmgr_init(&tomtom->resmgr, component, core_res, pdata,
	...
	tomtom->spkdrv_reg = tomtom_codec_find_regulator(component,
					       WCD9XXX_VDD_SPKDRV_NAME);
	tomtom->spkdrv2_reg = tomtom_codec_find_regulator(component,
					       WCD9XXX_VDD_SPKDRV2_NAME);
...
static const struct snd_soc_component_driver soc_codec_dev_tomtom = {
	.name = DRV_NAME,
...
static int tomtom_probe(struct platform_device *pdev)
{
	...
	if (wcd9xxx_get_intf_type() == WCD9XXX_INTERFACE_TYPE_SLIMBUS)
		ret = snd_soc_register_component(&pdev->dev, &soc_codec_dev_tomtom,
