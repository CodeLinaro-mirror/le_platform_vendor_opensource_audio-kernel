 ...
 *
 * @component: handle pointer to ep92 component
	...
	struct ep92_pdata *ep92)
{
	...
	if (change & EP92_GI_ARC_ON_MASK) {
		val = (ep92->gi.tx_info >> EP92_GI_ARC_ON_SHIFT) &
			EP92_2CHOICE_MASK;
		dev_dbg(component->dev, "ep92 arc_on changed to %d\n", val);
		if (val)
	...
	if (change & EP92_GI_EARC_ON_MASK) {
		val = (ep92->gi.tx_info >> EP92_GI_EARC_ON_SHIFT) &
			EP92_2CHOICE_MASK;
		dev_dbg(component->dev, "ep92 earc_on changed to %d\n", val);
		if (val)
	...
	if (change & EP92_GI_EARC_SEL_MASK) {
		val = (ep92->gi.tx_info >> EP92_GI_EARC_SEL_SHIFT) &
			EP92_2CHOICE_MASK;
		dev_dbg(component->dev, "ep92 earc_sel changed to %d\n", val);
		if (val)
		...
		if (val) {
			...
			snd_soc_component_write(ep92->component,
	...
	if (ep92->cur_earc_en != val) {
		dev_dbg(component->dev, "ep92 earc_en changed to %d\n", val);
		if (val)
	...
	case EP92_CHECK_START:
		dev_dbg(component->dev, "ep92 check initial wait\n");
	...
	case EP92_CHECK_START_WAIT: /* wait some time */
		if (ep92->check_cnt < ep92->check_wait_start) {
			ep92->check_cnt += EP92_POLL_INTERVAL_ON_MSEC;
			   ...
			   (ep92->gi.tx_info & EP92_GI_EARC_ON_MASK)) {
			dev_dbg(component->dev, "ep92 check already in eARC mode\n");
			ep92->check_state = EP92_CHECK_IDLE;
		} else if ((ep92->gc.ctl & EP92_GC_ARC_EN_MASK) &&
			   (ep92->gi.tx_info & EP92_GI_ARC_ON_MASK)) {
			dev_dbg(component->dev, "ep92 check already in ARC mode\n");
			ep92->check_state = EP92_CHECK_IDLE;
		} else if ((ep92->gi.tx_info & EP92_GI_EARC_SEL_MASK) == 0) {
			dev_dbg(component->dev, "ep92 check exit, as sel bit not set\n");
			ep92->check_state = EP92_CHECK_IDLE;
			...
			dev_dbg(component->dev, "ep92 check not enabled\n");
			ep92->check_state = EP92_CHECK_IDLE;
		}
		break;
	case EP92_CHECK_EARC_EN: /* set earc_en=1 (and arc_en=0) */
		dev_dbg(component->dev, "ep92 check for eARC\n");
	...
	case EP92_CHECK_EARC_WAIT: /* wait for earc_on or timeout */
		if (ep92->gi.tx_info & EP92_GI_EARC_ON_MASK) {
			...
			ep92->check_state = EP92_CHECK_IDLE;
			dev_dbg(component->dev, "ep92 in eARC mode\n");
		} else {
	...
	case EP92_CHECK_EARC_OFF: /* set earc_en=0 */
		dev_dbg(component->dev, "ep92 not in eARC mode\n");
		reg = snd_soc_component_read(ep92->component, EP92_GENERAL_CONTROL_0);
		reg &= ~EP92_GC_EARC_EN_MASK;
		snd_soc_component_write(ep92->component, EP92_GENERAL_CONTROL_0, reg);
		if (ep92->check_arc)
	...
	case EP92_CHECK_ARC_EN: /* set arc_en=1 (and earc_en=0) */
		dev_dbg(component->dev, "ep92 check for ARC\n");
		reg = snd_soc_component_read(ep92->component, EP92_GENERAL_CONTROL_0);
		reg &= ~EP92_GC_EARC_EN_MASK;
	...
	case EP92_CHECK_ARC_WAIT: /* wait for arc_on or timeout */
		if (ep92->gi.tx_info & EP92_GI_ARC_ON_MASK) {
			...
			ep92->check_state = EP92_CHECK_IDLE;
			dev_dbg(component->dev, "ep92 in ARC mode\n");
		} else {
	...
	case EP92_CHECK_ARC_OFF: /* set arc_en=0, repeat or stop sequence */
		...
		if (ep92->check_repeat_cnt < ep92->check_repeat) {
			dev_dbg(component->dev, "ep92 check repeat %d\n",
				ep92->check_repeat_cnt);
			ep92->check_state = EP92_CHECK_START_WAIT;
			ep92->check_cnt = 0;
		} else {
			dev_dbg(component->dev, "ep92 not in ARC mode\n");
			ep92->check_state = EP92_CHECK_IDLE;
		}
		break;
	...
	struct ep92_pdata *ep92)
{
	...
	if (ep92->mute_gpio) {
		...
		if (ep92->mute_active != old) {
			dev_dbg(component->dev, "ep92 mute pin changed to %d\n",
				ep92->mute_active);
			send_uevent = true;
		}
	}
	...
	if (change & EP92_AI_MCLK_ON_MASK) {
		dev_dbg(component->dev, "ep92 mclk changed to %d\n",
	...
	ep92->ai.system_status_1 = snd_soc_component_read(component,
	...
	if (change & EP92_AI_EARC_CON_MASK) {
		dev_dbg(component->dev, "ep92 earc_state changed to %d\n",
	...
	if (ep92->cur_audio_state != change) {
		dev_dbg(component->dev, "ep92 audio_state changed to %d\n", change);
		send_uevent = true;
	}
	...
	if (change & EP92_AI_COMP_ADO_MASK) {
		dev_dbg(component->dev, "ep92 earc layout changed to %d\n",
	...
	old = ep92->ai.cs[3];
	ep92->ai.cs[3] = snd_soc_component_read(component,
		EP92_AUDIO_INFO_CHANNEL_STATUS_3);
	if (ep92->ai.cs[3] == 0xff) {
		dev_dbg(component->dev, "ep92 EP92_AUDIO_INFO_CHANNEL_STATUS_3 read 0xff\n");
		ep92->ai.cs[3] = old;
	}
	change = ep92->ai.cs[3] ^ old;
	if (change & EP92_AI_EARC_RATE_MASK) {
		dev_dbg(component->dev, "ep92 earc_rate changed to %d\n",
		      ...
		      struct ep92_pdata *ep92)
{
	...
	dev_info(component->dev, "ep92 version info %02d/%02d/%02d %d\n",
		reg0, reg1, reg2, reg3);
...
static irqreturn_t ep92_irq(int irq, void *data)
{
	...
	dev_dbg(component->dev, "ep92_interrupt %d\n", irq);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	reg = snd_soc_component_read(ep92->component, EP92_GENERAL_CONTROL_2);
