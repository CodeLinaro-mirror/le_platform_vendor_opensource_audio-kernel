	...
	if (on && (++cp_count == 1)) {
		...
		if (--cp_count < 0) {
			dev_dbg(component->dev,
				...
				__func__);
			if (snd_soc_component_read32(component, WCD9XXX_A_CDC_CLK_OTHR_CTL) &
			    0x01) {
				dev_dbg(component->dev,
					...
					uint8_t value, int delay)
{
	...
	node->reg = reg;
	node->value = snd_soc_component_read32(component, reg);
