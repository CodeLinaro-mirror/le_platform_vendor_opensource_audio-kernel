...
static inline void
wcd9xxx_enable_clsh_block(struct snd_soc_component *component,
			  struct wcd9xxx_clsh_cdc_data *clsh_d, bool enable)
{
	if ((enable && ++clsh_d->clsh_users == 1) ||
	    (!enable && --clsh_d->clsh_users == 0))
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_CLSH_B1_CTL,
...
static inline void
wcd9xxx_enable_buck(struct snd_soc_component *component,
		    struct wcd9xxx_clsh_cdc_data *clsh_d, bool enable)
{
	if ((enable && ++clsh_d->buck_users == 1) ||
	    (!enable && --clsh_d->buck_users == 0))
		snd_soc_component_update_bits(component, WCD9XXX_A_BUCK_MODE_1,
				    0x80, enable ? 0x80 : 0x00);
	dev_dbg(component->dev, "%s: buck_users %d, enable %d", __func__,
		...
		struct snd_soc_component *component)
{
	...
	for (i = 0; i < ARRAY_SIZE(reg_set); i++)
		snd_soc_component_update_bits(component, reg_set[i].reg, reg_set[i].mask,
						    reg_set[i].val);

	dev_dbg(component->dev, "%s: Programmed class H controller common parameters",
			 __func__);
}

static void wcd9xxx_chargepump_request(struct snd_soc_component *component, bool on)
{
	static int cp_count;
	...
	if (on && (++cp_count == 1)) {
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_CLK_OTHR_CTL,
				    0x01, 0x01);
		dev_dbg(component->dev, "%s: Charge Pump enabled, count = %d\n",
			__func__, cp_count);
	} else if (!on) {
		if (--cp_count < 0) {
			dev_dbg(component->dev,
				...
				__func__);
			if (snd_soc_component_read(component, WCD9XXX_A_CDC_CLK_OTHR_CTL) &
			    0x01) {
				dev_dbg(component->dev,
...
void wcd9xxx_enable_high_perf_mode(struct snd_soc_component *component,
				struct wcd9xxx_clsh_cdc_data *clsh_d,
				u8 uhqa_mode, u8 req_state, bool req_type)
{
	dev_dbg(component->dev, "%s: users fclk8 %d, fclk5 %d", __func__,
			clsh_d->ncp_users[NCP_FCLK_LEVEL_8],
			clsh_d->ncp_users[NCP_FCLK_LEVEL_5]);

	if (req_type == WCD9XXX_CLSAB_REQ_ENABLE) {
		clsh_d->ncp_users[NCP_FCLK_LEVEL_8]++;
		snd_soc_component_write(component, WCD9XXX_A_RX_HPH_BIAS_PA,
					WCD9XXX_A_RX_HPH_BIAS_PA__POR);
		snd_soc_component_write(component, WCD9XXX_A_RX_HPH_L_PA_CTL, 0x48);
		snd_soc_component_write(component, WCD9XXX_A_RX_HPH_R_PA_CTL, 0x48);
		if (uhqa_mode)
			snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_CHOP_CTL,
						0x20, 0x00);
		wcd9xxx_chargepump_request(component, true);
		wcd9xxx_enable_anc_delay(component, true);
		wcd9xxx_enable_buck(component, clsh_d, false);
		if (clsh_d->ncp_users[NCP_FCLK_LEVEL_8] > 0)
			snd_soc_component_update_bits(component, WCD9XXX_A_NCP_STATIC,
						0x0F, 0x08);
		snd_soc_component_update_bits(component, WCD9XXX_A_NCP_STATIC, 0x30, 0x30);

		/* Enable NCP and wait until settles down */
		if (snd_soc_component_update_bits(component, WCD9XXX_A_NCP_EN, 0x01, 0x01))
			usleep_range(NCP_SETTLE_TIME_US, NCP_SETTLE_TIME_US+10);
	} else {
		snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_CHOP_CTL,
					0x20, 0x20);
		snd_soc_component_write(component, WCD9XXX_A_RX_HPH_L_PA_CTL,
					WCD9XXX_A_RX_HPH_L_PA_CTL__POR);
		snd_soc_component_write(component, WCD9XXX_A_RX_HPH_R_PA_CTL,
					WCD9XXX_A_RX_HPH_R_PA_CTL__POR);
		snd_soc_component_write(component, WCD9XXX_A_RX_HPH_BIAS_PA, 0x57);
		wcd9xxx_enable_buck(component, clsh_d, true);
		wcd9xxx_chargepump_request(component, false);
		wcd9xxx_enable_anc_delay(component, false);
		clsh_d->ncp_users[NCP_FCLK_LEVEL_8]--;
		if (clsh_d->ncp_users[NCP_FCLK_LEVEL_8] == 0 &&
		    clsh_d->ncp_users[NCP_FCLK_LEVEL_5] == 0)
			snd_soc_component_update_bits(component, WCD9XXX_A_NCP_EN,
						0x01, 0x00);
...
void wcd9xxx_clsh_imped_config(struct snd_soc_component *component,
				  int imped)
{
	int i  = 0;
	int index = 0;
	...
	for (i = 0; i < MAX_IMPED_PARAMS; i++)
		snd_soc_component_write(component, imped_table[index][i].reg,
					imped_table[index][i].val);
} EXPORT_SYMBOL(wcd9xxx_clsh_imped_config);

static void wcd9xxx_clsh_comp_req(struct snd_soc_component *component,
				  struct wcd9xxx_clsh_cdc_data *clsh_d,
				  int compute_pa, bool on)
{
	...
	if (compute_pa == CLSH_COMPUTE_EAR) {
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_CLSH_B1_CTL, 0x10,
				    (on ? 0x10 : 0));
	} else {
...
int wcd9xxx_soc_update_bits_push(struct snd_soc_component *component,
					...
					uint8_t value, int delay)
{
	...
	node->reg = reg;
	node->value = snd_soc_component_read(component, reg);
	list_add(&node->lh, list);
	if (mask == 0xFF)
		rc = snd_soc_component_write(component, reg, value);
	else
		rc = snd_soc_component_update_bits(component, reg, mask, value);
	if (delay)
		usleep_range(delay, delay + USLEEP_RANGE_MARGIN_US);
	return rc;
}
EXPORT_SYMBOL_GPL(wcd9xxx_soc_update_bits_push);

void wcd9xxx_restore_registers(struct snd_soc_component *component,
			       struct list_head *lh)
{
	struct wcd9xxx_register_save_node *node, *nodetmp;

	list_for_each_entry_safe(node, nodetmp, lh, lh) {
		snd_soc_component_write(component, node->reg, node->value);
		list_del(&node->lh);
		kfree(node);
	}
}
EXPORT_SYMBOL_GPL(wcd9xxx_restore_registers);

static void wcd9xxx_dynamic_bypass_buck_ctrl_lo(struct snd_soc_component *component,
						bool enable)
{
	int i;
	const struct wcd9xxx_reg_mask_val reg_set[] = {
	...
	if (!enable) {
		snd_soc_component_update_bits(component, WCD9XXX_A_BUCK_MODE_1,
					(0x1 << 3), 0x00);
		snd_soc_component_update_bits(component, WCD9XXX_A_BUCK_MODE_4,
					0xFF, BUCK_VREF_2V);
	}
	for (i = 0; i < ARRAY_SIZE(reg_set); i++)
		snd_soc_component_update_bits(component, reg_set[i].reg, reg_set[i].mask,
...
static void wcd9xxx_dynamic_bypass_buck_ctrl(struct snd_soc_component *component,
						bool enable)
{
	int i;
	const struct wcd9xxx_reg_mask_val reg_set[] = {
...
static void wcd9xxx_set_buck_mode(struct snd_soc_component *component, u8 buck_vref)
{
	int i;
	const struct wcd9xxx_reg_mask_val reg_set[] = {
	...
	for (i = 0; i < ARRAY_SIZE(reg_set); i++)
		snd_soc_component_update_bits(component, reg_set[i].reg,
...
static void wcd9xxx_clsh_enable_post_pa(struct snd_soc_component *component,
	struct wcd9xxx_clsh_cdc_data *cdc_clsh_d)
{
	...
	if (!cdc_clsh_d->is_dynamic_vdd_cp)
		snd_soc_component_update_bits(component, WCD9XXX_A_BUCK_MODE_3,
							0x08, 0x08);

	dev_dbg(component->dev, "%s: completed clsh mode settings after PA enable\n",
		   __func__);

}

static void wcd9xxx_set_fclk_get_ncp(struct snd_soc_component *component,
				     struct wcd9xxx_clsh_cdc_data *clsh_d,
				     enum ncp_fclk_level fclk_level)
{
	...
	snd_soc_component_update_bits(component, WCD9XXX_A_NCP_STATIC, 0x10, 0x00);
	/* fclk level 8 dominates level 5 */
	if (clsh_d->ncp_users[NCP_FCLK_LEVEL_8] > 0)
		snd_soc_component_update_bits(component, WCD9XXX_A_NCP_STATIC, 0x0F, 0x08);
	else if (clsh_d->ncp_users[NCP_FCLK_LEVEL_5] > 0)
		snd_soc_component_update_bits(component, WCD9XXX_A_NCP_STATIC, 0x0F, 0x05);
	else
		WARN_ONCE(1, "Unexpected users %d,%d\n",
			  clsh_d->ncp_users[NCP_FCLK_LEVEL_8],
			  clsh_d->ncp_users[NCP_FCLK_LEVEL_5]);
	snd_soc_component_update_bits(component, WCD9XXX_A_NCP_STATIC, 0x20, 0x20);
...
static void wcd9xxx_set_fclk_put_ncp(struct snd_soc_component *component,
				     struct wcd9xxx_clsh_cdc_data *clsh_d,
				     enum ncp_fclk_level fclk_level)
{
	clsh_d->ncp_users[fclk_level]--;
	...
	if (clsh_d->ncp_users[NCP_FCLK_LEVEL_8] == 0 &&
	    clsh_d->ncp_users[NCP_FCLK_LEVEL_5] == 0)
		snd_soc_component_update_bits(component, WCD9XXX_A_NCP_EN, 0x01, 0x00);
	else if (clsh_d->ncp_users[NCP_FCLK_LEVEL_8] == 0)
		/* if dominating level 8 has gone, switch to 5 */
		snd_soc_component_update_bits(component, WCD9XXX_A_NCP_STATIC, 0x0F, 0x05);
}

static void wcd9xxx_cfg_clsh_param_ear(struct snd_soc_component *component)
{
	int i;
	const struct wcd9xxx_reg_mask_val reg_set[] = {
	...
	dev_dbg(component->dev, "%s: Programmed Class H controller EAR specific params\n",
			 __func__);
}

static void wcd9xxx_cfg_clsh_param_hph(struct snd_soc_component *component)
{
	int i;
	const struct wcd9xxx_reg_mask_val reg_set[] = {
	...
	for (i = 0; i < ARRAY_SIZE(reg_set); i++)
		snd_soc_component_update_bits(component, reg_set[i].reg, reg_set[i].mask,
							reg_set[i].val);
	dev_dbg(component->dev, "%s: Programmed Class H controller HPH specific params\n",
			 __func__);
}

static void wcd9xxx_ncp_bypass_enable(struct snd_soc_component *component, bool enable)
{
	snd_soc_component_update_bits(component, WCD9XXX_A_NCP_STATIC, 0x10, (enable << 4));
	/* 50us sleep is reqd. as per the class H HW design sequence */
	usleep_range(BUCK_SETTLE_TIME_US, BUCK_SETTLE_TIME_US+10);
}

static void wcd9xxx_clsh_set_Iest(struct snd_soc_component *component,
		u8 value)
{
	snd_soc_component_update_bits(component, WCD9XXX_A_BUCK_MODE_5,
				    0x01, (0x01 & 0x03));
	snd_soc_component_update_bits(component, WCD9XXX_A_BUCK_MODE_5,
				    0xFC, (value << 2));
}

static void wcd9xxx_clsh_state_hph_ear(struct snd_soc_component *component,
			struct wcd9xxx_clsh_cdc_data *clsh_d,
			u8 req_state, bool is_enable)
{
	int compute_pa = 0;

	dev_dbg(component->dev, "%s: enter %s\n", __func__,
	...
	if (is_enable) {
		...
		if ((clsh_d->state == WCD9XXX_CLSH_STATE_EAR) ||
			...
			wcd9xxx_dynamic_bypass_buck_ctrl(component, false);
			wcd9xxx_ncp_bypass_enable(component, true);
		}
		switch (req_state) {
		...
		wcd9xxx_clsh_comp_req(component, clsh_d, compute_pa, true);

		dev_dbg(component->dev, "%s: Enabled hph+ear mode clsh\n",
				__func__);
	} else {
		switch (req_state) {
		case WCD9XXX_CLSH_STATE_HPHL:
		...
		wcd9xxx_clsh_comp_req(component, clsh_d, compute_pa, false);

		if (((clsh_d->state & (~req_state)) ==
				WCD9XXX_CLSH_STATE_EAR) ||
			(req_state == WCD9XXX_CLSH_STATE_EAR)) {
			wcd9xxx_ncp_bypass_enable(component, false);
			wcd9xxx_dynamic_bypass_buck_ctrl(component, true);
		}
	}
}

static void wcd9xxx_clsh_state_hph_lo(struct snd_soc_component *component,
			struct wcd9xxx_clsh_cdc_data *clsh_d,
			u8 req_state, bool is_enable)
{
	...
	if (is_enable) {
		if ((clsh_d->state == WCD9XXX_CLSH_STATE_LO) ||
			...
			wcd9xxx_dynamic_bypass_buck_ctrl_lo(component, false);
			wcd9xxx_enable_buck(component, clsh_d, true);
			wcd9xxx_ncp_bypass_enable(component, true);
			if (req_state & WCD9XXX_CLSH_STATE_HPH_ST) {
				wcd9xxx_set_fclk_get_ncp(component, clsh_d,
							NCP_FCLK_LEVEL_8);
				wcd9xxx_set_fclk_put_ncp(component, clsh_d,
							NCP_FCLK_LEVEL_5);
				wcd9xxx_enable_clsh_block(component, clsh_d, true);
				wcd9xxx_chargepump_request(component, true);
				wcd9xxx_enable_anc_delay(component, true);
			}
		}
		if (req_state == WCD9XXX_CLSH_STATE_HPHL)
			wcd9xxx_clsh_comp_req(component, clsh_d,
		...
		case WCD9XXX_CLSH_STATE_LO:
			snd_soc_component_update_bits(component, WCD9XXX_A_NCP_STATIC,
						0x20, 0x00);
			wcd9xxx_dynamic_bypass_buck_ctrl_lo(component, true);
			break;
		case WCD9XXX_CLSH_STATE_HPHL:
			wcd9xxx_clsh_comp_req(component, clsh_d,
		...
		((clsh_d->state & (~req_state)) == WCD9XXX_CLSH_STATE_LO)) {
			...
			if (req_state & WCD9XXX_CLSH_STATE_HPH_ST) {
				...
				if (clsh_d->buck_mv ==
					...
					wcd9xxx_enable_buck(component, clsh_d,
								false);
					wcd9xxx_ncp_bypass_enable(component, true);
				} else {
					/*
					...
					wcd9xxx_clsh_set_Iest(component, 0x02);
				}
...
static void wcd9xxx_clsh_state_ear_lo(struct snd_soc_component *component,
			struct wcd9xxx_clsh_cdc_data *clsh_d,
			u8 req_state, bool is_enable)
{
	...
	if (is_enable) {
		...
		if (req_state & WCD9XXX_CLSH_STATE_LO) {
			...
			wcd9xxx_clsh_comp_req(component, clsh_d, CLSH_COMPUTE_EAR,
						false);
			/*sleep 5ms*/
...
static void wcd9xxx_clsh_state_hph_ear_lo(struct snd_soc_component *component,
			struct wcd9xxx_clsh_cdc_data *clsh_d,
			u8 req_state, bool is_enable)
{
	dev_dbg(component->dev, "%s: enter %s\n", __func__,
	...
	if (req_state & WCD9XXX_CLSH_STATE_HPHL)
		wcd9xxx_clsh_comp_req(component, clsh_d, CLSH_COMPUTE_HPH_L,
					is_enable);

	if (req_state & WCD9XXX_CLSH_STATE_HPHR)
		wcd9xxx_clsh_comp_req(component, clsh_d, CLSH_COMPUTE_HPH_R,
					is_enable);
...
static void wcd9xxx_clsh_state_ear(struct snd_soc_component *component,
			struct wcd9xxx_clsh_cdc_data *clsh_d,
			u8 req_state, bool is_enable)
{
	pr_debug("%s: enter %s\n", __func__, is_enable ? "enable" : "disable");
	if (is_enable) {
		wcd9xxx_cfg_clsh_param_common(component);
		wcd9xxx_cfg_clsh_param_ear(component);
		wcd9xxx_enable_clsh_block(component, clsh_d, true);
		wcd9xxx_chargepump_request(component, true);
		wcd9xxx_enable_anc_delay(component, true);
		wcd9xxx_clsh_comp_req(component, clsh_d, CLSH_COMPUTE_EAR, true);
		wcd9xxx_set_buck_mode(component, BUCK_VREF_2V);
		wcd9xxx_enable_buck(component, clsh_d, true);
		wcd9xxx_set_fclk_get_ncp(component, clsh_d, NCP_FCLK_LEVEL_8);

		dev_dbg(component->dev, "%s: Enabled ear mode class h\n", __func__);
	} else {
		dev_dbg(component->dev, "%s: stub fallback to ear\n", __func__);
		wcd9xxx_set_fclk_put_ncp(component, clsh_d, NCP_FCLK_LEVEL_8);
		wcd9xxx_enable_buck(component, clsh_d, false);
		wcd9xxx_clsh_comp_req(component, clsh_d, CLSH_COMPUTE_EAR, false);
		wcd9xxx_chargepump_request(component, false);
		wcd9xxx_enable_clsh_block(component, clsh_d, false);
	}
}

static void wcd9xxx_clsh_state_hph_l(struct snd_soc_component *component,
		struct wcd9xxx_clsh_cdc_data *clsh_d,
		u8 req_state, bool is_enable)
{
	pr_debug("%s: enter %s\n", __func__, is_enable ? "enable" : "disable");
	...
	if (is_enable) {
		wcd9xxx_cfg_clsh_param_common(component);
		wcd9xxx_cfg_clsh_param_hph(component);
		wcd9xxx_enable_clsh_block(component, clsh_d, true);
		wcd9xxx_chargepump_request(component, true);
		wcd9xxx_enable_anc_delay(component, true);
		wcd9xxx_clsh_comp_req(component, clsh_d, CLSH_COMPUTE_HPH_L, true);
		wcd9xxx_clsh_comp_req(component, clsh_d, CLSH_COMPUTE_HPH_R, true);
		wcd9xxx_set_buck_mode(component, BUCK_VREF_0P494V);
		wcd9xxx_enable_buck(component, clsh_d, true);
		wcd9xxx_set_fclk_get_ncp(component, clsh_d, NCP_FCLK_LEVEL_8);
		...
		wcd9xxx_set_fclk_put_ncp(component, clsh_d, NCP_FCLK_LEVEL_8);
		wcd9xxx_enable_buck(component, clsh_d, false);
		wcd9xxx_clsh_comp_req(component, clsh_d, CLSH_COMPUTE_HPH_L, false);
		wcd9xxx_clsh_comp_req(component, clsh_d, CLSH_COMPUTE_HPH_R, false);
		wcd9xxx_enable_clsh_block(component, clsh_d, false);
		wcd9xxx_chargepump_request(component, false);
	}
}

static void wcd9xxx_clsh_state_hph_r(struct snd_soc_component *component,
		struct wcd9xxx_clsh_cdc_data *clsh_d,
		u8 req_state, bool is_enable)
{
	pr_debug("%s: enter %s\n", __func__, is_enable ? "enable" : "disable");
...
static void wcd9xxx_clsh_state_hph_st(struct snd_soc_component *component,
		struct wcd9xxx_clsh_cdc_data *clsh_d,
		u8 req_state, bool is_enable)
{
	pr_debug("%s: enter %s\n", __func__, is_enable ? "enable" : "disable");
	...
	if (is_enable)
		dev_dbg(component->dev, "%s: stub fallback to hph_st\n", __func__);
	else
		dev_dbg(component->dev, "%s: stub fallback to hph_st\n", __func__);
}

static void wcd9xxx_clsh_state_lo(struct snd_soc_component *component,
		struct wcd9xxx_clsh_cdc_data *clsh_d,
		u8 req_state, bool is_enable)
{
	...
	if (is_enable) {
		wcd9xxx_set_buck_mode(component, BUCK_VREF_1P8V);
		wcd9xxx_enable_buck(component, clsh_d, true);
		wcd9xxx_set_fclk_get_ncp(component, clsh_d, NCP_FCLK_LEVEL_5);
		...
		snd_soc_component_update_bits(component, WCD9XXX_A_BUCK_MODE_1, 0x04, 0x00);
	} else {
		dev_dbg(component->dev, "%s: stub fallback to lineout\n", __func__);
		wcd9xxx_set_fclk_put_ncp(component, clsh_d, NCP_FCLK_LEVEL_5);
		if (clsh_d->buck_mv != WCD9XXX_CDC_BUCK_MV_1P8)
			wcd9xxx_enable_buck(component, clsh_d, false);
	}
}

static void wcd9xxx_clsh_state_err(struct snd_soc_component *component,
		struct wcd9xxx_clsh_cdc_data *clsh_d,
		u8 req_state, bool is_enable)
{
	char msg[128];
 ...
 */
void wcd9xxx_clsh_fsm(struct snd_soc_component *component,
		struct wcd9xxx_clsh_cdc_data *cdc_clsh_d,
		u8 req_state, bool req_type, u8 clsh_event)
{
	...
	case WCD9XXX_CLSH_EVENT_PRE_DAC:
		...
		WARN_ON_ONCE(req_type != WCD9XXX_CLSH_REQ_ENABLE);
	...
	case WCD9XXX_CLSH_EVENT_POST_PA:
		if (req_type == WCD9XXX_CLSH_REQ_DISABLE) {
			...
			if (new_state < NUM_CLSH_STATES) {
				...
				dev_dbg(component->dev, "%s:wrong new state=0x%x\n",
						__func__, new_state);
			}
		} else if (!(cdc_clsh_d->state & WCD9XXX_CLSH_STATE_LO)) {
			wcd9xxx_clsh_enable_post_pa(component, cdc_clsh_d);
		}
