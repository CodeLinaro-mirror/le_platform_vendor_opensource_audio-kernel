...
static void wcd9xxx_enable_bg_mbhc(struct wcd9xxx_resmgr *resmgr)
{
	 ...
	 */
	WARN_ON(snd_soc_component_read(component, WCD9XXX_A_CLK_BUFF_EN2) & (1 << 2));
				...
				enum wcd9xxx_clock_config_mode config_mode)
{
	...
	if (config_mode == WCD9XXX_CFG_RCO) {
		...
		if (resmgr->pdata->mclk_rate == WCD9XXX_MCLK_CLK_12P288MHZ) {
			...
			snd_soc_component_update_bits(component, TOMTOM_A_RCO_CTRL,
					0x18, 0x10);
			valr = snd_soc_component_read(component,
					TOMTOM_A_QFUSE_DATA_OUT0) & (0x04);
			valr1 = snd_soc_component_read(component,
		...
		default:
			...
			if (snd_soc_component_read(component, WCD9XXX_A_RC_OSC_FREQ) & 0x80) {
	...
	case WCD9XXX_CLK_MCLK:
		if (++resmgr->clk_mclk_users == 1 &&
			...
			if (resmgr->codec_type == WCD9XXX_CDC_TYPE_TOMTOM) {
				...
				/* if RCO is enabled, switch from it */
				WARN_ON(!(snd_soc_component_read(resmgr->component,
				  ...
				  enum wcd9xxx_clock_type type)
{
	...
	case WCD9XXX_CLK_RCO:
		if (--resmgr->clk_rco_users == 0 &&
			...
			if (resmgr->codec_type == WCD9XXX_CDC_TYPE_TOMTOM) {
				...
				/* if RCO is enabled, switch from it */
				if (snd_soc_component_read(resmgr->component,
	...
	case WCD9XXX_CLK_MCLK:
		if (--resmgr->clk_mclk_users == 0 &&
			...
			wcd9xxx_disable_clock_block(resmgr);
			if ((resmgr->codec_type == WCD9XXX_CDC_TYPE_TOMTOM) &&
					(snd_soc_component_read(component, TOMTOM_A_RCO_CTRL)
					 & 0x80)) {
				/* powerdown RCO*/
				snd_soc_component_update_bits(component, TOMTOM_A_RCO_CTRL,
						0x80, 0x00);
			...
			if (resmgr->codec_type == WCD9XXX_CDC_TYPE_TOMTOM) {
				if (!(snd_soc_component_read(component, TOMTOM_A_RCO_CTRL) &
				      0x80)) {
