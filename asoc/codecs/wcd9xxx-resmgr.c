...
}EXPORT_SYMBOL(wcd9xxx_resmgr_notifier_call);
...
static void wcd9xxx_disable_bg(struct wcd9xxx_resmgr *resmgr)
{
	...
	/* Disable bg */
	snd_soc_component_update_bits(resmgr->component, WCD9XXX_A_BIAS_CENTRAL_BG_CTL,
			    0x03, 0x00);
...
{
	struct snd_soc_component *component = resmgr->component;

	/* Enable BG in slow mode and precharge */
	snd_soc_component_update_bits(component, WCD9XXX_A_BIAS_CENTRAL_BG_CTL, 0x80, 0x80);
	snd_soc_component_update_bits(component, WCD9XXX_A_BIAS_CENTRAL_BG_CTL, 0x04, 0x04);
	snd_soc_component_update_bits(component, WCD9XXX_A_BIAS_CENTRAL_BG_CTL, 0x01, 0x01);
	usleep_range(1000, 1100);
	snd_soc_component_update_bits(component, WCD9XXX_A_BIAS_CENTRAL_BG_CTL, 0x80, 0x00);
}
...
static void wcd9xxx_enable_bg_mbhc(struct wcd9xxx_resmgr *resmgr)
{
	 ...
	 */
	WARN_ON(snd_soc_component_read32(component, WCD9XXX_A_CLK_BUFF_EN2) & (1 << 2));
...
static void wcd9xxx_disable_clock_block(struct wcd9xxx_resmgr *resmgr)
{
	...
	case WCD9XXX_CDC_TYPE_TOMTOM:
		snd_soc_component_update_bits(component, WCD9XXX_A_CLK_BUFF_EN2, 0x04, 0x00);
		usleep_range(50, 55);
		snd_soc_component_update_bits(component, WCD9XXX_A_CLK_BUFF_EN2, 0x02, 0x02);
		snd_soc_component_update_bits(component, WCD9XXX_A_CLK_BUFF_EN1, 0x40, 0x40);
		snd_soc_component_update_bits(component, WCD9XXX_A_CLK_BUFF_EN1, 0x40, 0x00);
		snd_soc_component_update_bits(component, WCD9XXX_A_CLK_BUFF_EN1, 0x01, 0x00);
		break;
	default:
		snd_soc_component_update_bits(component, WCD9XXX_A_CLK_BUFF_EN2, 0x04, 0x00);
		usleep_range(50, 55);
		snd_soc_component_update_bits(component, WCD9XXX_A_CLK_BUFF_EN2, 0x02, 0x02);
		snd_soc_component_update_bits(component, WCD9XXX_A_CLK_BUFF_EN1, 0x05, 0x00);
		break;
	}
	usleep_range(50, 55);
	/* Notify */
						...
						int clk_users)
{
	...
	if (clk_users) {
		if (resmgr->resmgr_cb &&
			...
			while (clk_users--)
				resmgr->resmgr_cb->cdc_rco_ctrl(resmgr->component,
								true);
		}
	}
...
void wcd9xxx_resmgr_post_ssr(struct wcd9xxx_resmgr *resmgr)
{
	...
	WCD9XXX_BG_CLK_UNLOCK(resmgr);
	pr_debug("%s: leave\n", __func__);
}EXPORT_SYMBOL(wcd9xxx_resmgr_post_ssr);
...
}EXPORT_SYMBOL(wcd9xxx_resmgr_get_bandgap);
				...
				enum wcd9xxx_bandgap_type choice)
{
	...
	pr_debug("%s: bg users audio %d, mbhc %d\n", __func__,
		 resmgr->bg_audio_users, resmgr->bg_mbhc_users);
}EXPORT_SYMBOL(wcd9xxx_resmgr_put_bandgap);
...
}EXPORT_SYMBOL(wcd9xxx_resmgr_enable_rx_bias);
...
int wcd9xxx_resmgr_enable_config_mode(struct wcd9xxx_resmgr *resmgr, int enable)
{
	...
	pr_debug("%s: enable = %d\n", __func__, enable);
	if (enable) {
		snd_soc_component_update_bits(component, WCD9XXX_A_RC_OSC_FREQ, 0x10, 0);
		/* bandgap mode to fast */
		if (resmgr->pdata->mclk_rate == WCD9XXX_MCLK_CLK_12P288MHZ)
			/* Set current value to 200nA for 12.288MHz clock */
			snd_soc_component_write(component, WCD9XXX_A_BIAS_OSC_BG_CTL, 0x37);
		else
			snd_soc_component_write(component, WCD9XXX_A_BIAS_OSC_BG_CTL, 0x17);

		usleep_range(5, 10);
		snd_soc_component_update_bits(component, WCD9XXX_A_RC_OSC_FREQ, 0x80, 0x80);
		snd_soc_component_update_bits(component, WCD9XXX_A_RC_OSC_TEST, 0x80, 0x80);
		usleep_range(10, 20);
		snd_soc_component_update_bits(component, WCD9XXX_A_RC_OSC_TEST, 0x80, 0);
		usleep_range(10000, 10100);

		if (resmgr->pdata->mclk_rate != WCD9XXX_MCLK_CLK_12P288MHZ)
			snd_soc_component_update_bits(component, WCD9XXX_A_CLK_BUFF_EN1,
							0x08, 0x08);
	} else {
		snd_soc_component_update_bits(component, WCD9XXX_A_BIAS_OSC_BG_CTL, 0x1, 0);
		snd_soc_component_update_bits(component, WCD9XXX_A_RC_OSC_FREQ, 0x80, 0);
	}
				...
				enum wcd9xxx_clock_config_mode config_mode)
{
	...
	if (config_mode == WCD9XXX_CFG_RCO) {
		...
		snd_soc_component_write(component, WCD9XXX_A_CLK_BUFF_EN2, 0x02);
		usleep_range(1000, 1100);
	} else if (config_mode == WCD9XXX_CFG_CAL_RCO) {
		snd_soc_component_update_bits(component, TOMTOM_A_BIAS_OSC_BG_CTL,
				    0x01, 0x01);
		...
		if (resmgr->pdata->mclk_rate == WCD9XXX_MCLK_CLK_12P288MHZ) {
			 ...
			 */
			snd_soc_component_update_bits(component, TOMTOM_A_RCO_CTRL,
					0x50, 0x50);
		} else {
			snd_soc_component_update_bits(component, TOMTOM_A_RCO_CTRL,
					0x18, 0x10);
			valr = snd_soc_component_read32(component,
					TOMTOM_A_QFUSE_DATA_OUT0) & (0x04);
			valr1 = snd_soc_component_read32(component,
					TOMTOM_A_QFUSE_DATA_OUT1) & (0x08);
			valr = (valr >> 1) | (valr1 >> 3);
			snd_soc_component_update_bits(component, TOMTOM_A_RCO_CTRL, 0x60,
					valw[valr] << 5);
		}
		snd_soc_component_update_bits(component, TOMTOM_A_RCO_CTRL, 0x80, 0x80);
		...
		default:
			...
			if (snd_soc_component_read32(component, WCD9XXX_A_RC_OSC_FREQ) & 0x80) {
				snd_soc_component_write(component, WCD9XXX_A_CLK_BUFF_EN2,
					      0x02);
	...
	if (config_mode != WCD9XXX_CFG_CAL_RCO) {
		...
		snd_soc_component_update_bits(component, WCD9XXX_A_CLK_BUFF_EN2,
				    0x02, 0x00);
		/* on MCLK */
		snd_soc_component_update_bits(component, WCD9XXX_A_CLK_BUFF_EN2,
				    0x04, 0x04);
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_CLK_MCLK_CTL,
				    0x01, 0x01);
	}
	usleep_range(50, 55);
				  ...
				  enum wcd9xxx_clock_type type)
{
	...
	case WCD9XXX_CLK_MCLK:
		if (++resmgr->clk_mclk_users == 1 &&
			...
			if (resmgr->codec_type == WCD9XXX_CDC_TYPE_TOMTOM) {
				...
				/* if RCO is enabled, switch from it */
				WARN_ON(!(snd_soc_component_read32(resmgr->component,
	...
	pr_debug("%s: leave\n", __func__);
}EXPORT_SYMBOL(wcd9xxx_resmgr_get_clk_block);
				  ...
				  enum wcd9xxx_clock_type type)
{
	...
	case WCD9XXX_CLK_RCO:
		if (--resmgr->clk_rco_users == 0 &&
			...
			if (resmgr->codec_type == WCD9XXX_CDC_TYPE_TOMTOM) {
				...
				/* if RCO is enabled, switch from it */
				if (snd_soc_component_read32(resmgr->component,
						 WCD9XXX_A_RC_OSC_FREQ)
						 & 0x80) {
					snd_soc_component_write(resmgr->component,
	...
	case WCD9XXX_CLK_MCLK:
		if (--resmgr->clk_mclk_users == 0 &&
			...
			if ((resmgr->codec_type == WCD9XXX_CDC_TYPE_TOMTOM) &&
			    (snd_soc_component_read32(component, TOMTOM_A_RCO_CTRL) & 0x80)) {
				/* powerdown RCO*/
				 snd_soc_component_update_bits(component, TOMTOM_A_RCO_CTRL,
						     0x80, 0x00);
				 snd_soc_component_update_bits(component,
			...
			if (resmgr->codec_type == WCD9XXX_CDC_TYPE_TOMTOM) {
				if (!(snd_soc_component_read32(component, TOMTOM_A_RCO_CTRL) &
				      0x80)) {
					dev_dbg(component->dev, "%s: Enabling RCO\n",
						__func__);
					wcd9xxx_enable_clock_block(resmgr,
							WCD9XXX_CFG_CAL_RCO);
					snd_soc_component_update_bits(component,
	...
	pr_debug("%s: new rco_users %d, mclk_users %d\n", __func__,
		 resmgr->clk_rco_users, resmgr->clk_mclk_users);
}EXPORT_SYMBOL(wcd9xxx_resmgr_put_clk_block);
...
}EXPORT_SYMBOL(wcd9xxx_resmgr_get_clk_type);
					      ...
					      bool inc)
{
	...
	if (inc) {
		if ((resmgr->cfilt_users[cfilt_sel]++) == 0) {
			...
			/* Enable CFILT */
			snd_soc_component_update_bits(component, micb_cfilt_reg, 0x80, 0x80);
		}
	} else {
		/*
		...
		if ((--resmgr->cfilt_users[cfilt_sel]) == 0) {
			/* Disable CFILT */
			snd_soc_component_update_bits(component, micb_cfilt_reg, 0x80, 0);
...
}EXPORT_SYMBOL(wcd9xxx_resmgr_cfilt_get);
...
}EXPORT_SYMBOL(wcd9xxx_resmgr_cfilt_put);
			     ...
			     unsigned int cfilt_mv)
{
	...
	return rc;
}EXPORT_SYMBOL(wcd9xxx_resmgr_get_k_val);
					     ...
					     enum wcd9xxx_resmgr_cond cond)
{
	...
	list_for_each(l, &resmgr->update_bit_cond_h) {
		...
		if (e->cond == cond)
			snd_soc_component_update_bits(resmgr->component, e->reg,
					    1 << e->shift,
					...
					bool invert)
{
	...
	return 0;
}EXPORT_SYMBOL(wcd9xxx_resmgr_add_cond_update_bits);
				       ...
				       bool invert)
{
	...
	return -EINVAL;
}EXPORT_SYMBOL(wcd9xxx_resmgr_rm_cond_update_bits);
			...
			enum wcd9xxx_cdc_type cdc_type)
{
	...
	return 0;
}EXPORT_SYMBOL(wcd9xxx_resmgr_init);
...
}EXPORT_SYMBOL(wcd9xxx_resmgr_deinit);
