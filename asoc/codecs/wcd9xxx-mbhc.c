...
{
	return snd_soc_component_read(mbhc->component, WCD9XXX_A_CDC_MBHC_EN_CTL) & 0x1;
}
...
static void wcd9xxx_turn_onoff_override(struct wcd9xxx_mbhc *mbhc, bool on)
{
	...
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B1_CTL,
			    0x04, on ? 0x04 : 0x00);
}
...
static void wcd9xxx_pause_hs_polling(struct wcd9xxx_mbhc *mbhc)
{
	...
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x8, 0x8);
}
...
static void wcd9xxx_start_hs_polling(struct wcd9xxx_mbhc *mbhc)
{
	...
	if (mbhc->mbhc_cfg->use_int_rbias) {
		if (mbhc->mbhc_cb && mbhc->mbhc_cb->setup_int_rbias)
			mbhc->mbhc_cb->setup_int_rbias(component, true);
		else
			pr_err("%s: internal bias requested but codec did not provide callback\n",
				__func__);
	}

	snd_soc_component_write(component, WCD9XXX_A_MBHC_SCALING_MUX_1, 0x04);
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mux_bias_block)
		mbhc->mbhc_cb->enable_mux_bias_block(component);
	else
		snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_SCALING_MUX_1,
				    0x80, 0x80);
	...
	if (!mbhc->no_mic_headset_override &&
		...
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B2_CTL, 0x7F);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B1_CTL, 0xFF);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B4_CTL, 0x7F);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B3_CTL, 0xFF);
		/* set to max */
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B6_CTL, 0x7F);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B5_CTL, 0xFF);
	}

	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_EN_CTL, 0x1);
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x8, 0x0);
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_EN_CTL, 0x1);
}
				     ...
				     bool checkpolling)
{
	...
	if (vddio_switch && !mbhc->mbhc_micbias_switched &&
		...
		override = snd_soc_component_read(component, WCD9XXX_A_CDC_MBHC_B1_CTL) &
			   0x04;
		if (!override)
		...
		if (d->micb_mv != VDDIO_MICBIAS_MV) {
			...
			snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B1_CTL,
				      d->v_ins_hu[MBHC_V_IDX_VDDIO] & 0xFF);
			snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B2_CTL,
		...
		snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.mbhc_reg,
				    0x80, 0x80);
		snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.mbhc_reg,
				    0x10, 0x00);
		if (!override)
		...
		snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.mbhc_reg, 0x80,
				    0x00);
		snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.mbhc_reg, 0x10,
				    0x00);
...
static void wcd9xxx_calibrate_hs_polling(struct wcd9xxx_mbhc *mbhc)
{
	...
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B1_CTL, v_ins_hu & 0xFF);
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B2_CTL,
		      (v_ins_hu >> 8) & 0xFF);

	if (mbhc->mbhc_state != MBHC_STATE_POTENTIAL_RECOVERY) {
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B3_CTL, v_b1_hu &
				0xFF);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B4_CTL,
				(v_b1_hu >> 8) & 0xFF);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B5_CTL, v_b1_h &
				0xFF);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B6_CTL,
				(v_b1_h >> 8) & 0xFF);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B9_CTL, v_brh &
				0xFF);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B10_CTL,
				(v_brh >> 8) & 0xFF);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B11_CTL,
				mbhc->mbhc_data.v_brl & 0xFF);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_VOLT_B12_CTL,
					    ...
					    bool fast)
{
	...
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->switch_cfilt_mode) {
		...
		cfilt_mode.cur_mode_val =
		    snd_soc_component_read(component, mbhc->mbhc_bias_regs.cfilt_ctl) & 0x40;
	}
				...
				int irq)
{
	...
	if (mbhc->hph_status & jack_status) {
		...
		snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_OCP_CTL, 0x10,
				    0x00);
		snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_OCP_CTL, 0x10,
				    0x10);
		/*
...
static void wcd9xxx_clr_and_turnon_hph_padac(struct wcd9xxx_mbhc *mbhc)
{
	...
	wg_time = snd_soc_component_read(component, WCD9XXX_A_RX_HPH_CNP_WG_TIME);
	wg_time += 1;

	if (test_and_clear_bit(WCD9XXX_HPHR_DAC_OFF_ACK,
			       &mbhc->hph_pa_dac_state)) {
		pr_debug("%s: HPHR clear flag and enable DAC\n", __func__);
		snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_R_DAC_CTL,
				    0xC0, 0xC0);
	}
	if (test_and_clear_bit(WCD9XXX_HPHL_DAC_OFF_ACK,
				&mbhc->hph_pa_dac_state)) {
		pr_debug("%s: HPHL clear flag and enable DAC\n", __func__);
		snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_L_DAC_CTL,
				    0x80, 0x80);
	}
	...
	if (test_and_clear_bit(WCD9XXX_HPHR_PA_OFF_ACK,
		...
		snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_CNP_EN, 0x10,
				    1 << 4);
		pa_turned_on = true;
	}
	if (test_and_clear_bit(WCD9XXX_HPHL_PA_OFF_ACK,
		...
		snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_CNP_EN, 0x20, 1
				    << 5);
		pa_turned_on = true;
	}
...
static bool wcd9xxx_is_hph_dac_on(struct snd_soc_component *component, int left)
{
	u8 hph_reg_val = 0;

	if (left)
		hph_reg_val = snd_soc_component_read(component, WCD9XXX_A_RX_HPH_L_DAC_CTL);
	else
		hph_reg_val = snd_soc_component_read(component, WCD9XXX_A_RX_HPH_R_DAC_CTL);

	return (hph_reg_val & 0xC0) ? true : false;
}

static bool wcd9xxx_is_hph_pa_on(struct snd_soc_component *component)
{
	u8 hph_reg_val = 0;

	hph_reg_val = snd_soc_component_read(component, WCD9XXX_A_RX_HPH_CNP_EN);
...
static void wcd9xxx_set_and_turnoff_hph_padac(struct wcd9xxx_mbhc *mbhc)
{
	 ...
	 */
	if (wcd9xxx_is_hph_pa_on(component)) {
	...
	if (wcd9xxx_is_hph_dac_on(component, 1))
		set_bit(WCD9XXX_HPHL_DAC_OFF_ACK, &mbhc->hph_pa_dac_state);
	if (wcd9xxx_is_hph_dac_on(component, 0))
		set_bit(WCD9XXX_HPHR_DAC_OFF_ACK, &mbhc->hph_pa_dac_state);

	snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_CNP_EN, 0x30, 0x00);
	snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_L_DAC_CTL, 0x80, 0x00);
	snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_R_DAC_CTL, 0xC0, 0x00);
	usleep_range(wg_time * 1000, wg_time * 1000 + 50);
}
...
static void wcd9xxx_insert_detect_setup(struct wcd9xxx_mbhc *mbhc, bool ins)
{
	...
	snd_soc_component_update_bits(mbhc->component, WCD9XXX_A_MBHC_INSERT_DETECT, 1, 0);
	if (mbhc->mbhc_cfg->gpio_level_insert)
		snd_soc_component_write(mbhc->component, WCD9XXX_A_MBHC_INSERT_DETECT,
			      (0x68 | (ins ? (1 << 1) : 0)));
	else
		snd_soc_component_write(mbhc->component, WCD9XXX_A_MBHC_INSERT_DETECT,
			      (0x6C | (ins ? (1 << 1) : 0)));
	/* Re-enable detection */
	snd_soc_component_update_bits(mbhc->component, WCD9XXX_A_MBHC_INSERT_DETECT, 1, 1);
}
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		if (mbhc->micbias_enable && mbhc->micbias_enable_cb) {
			...
			mbhc->micbias_enable_cb(mbhc->component, false,
		...
		if (mbhc->mbhc_cb && mbhc->mbhc_cb->hph_auto_pulldown_ctrl)
			mbhc->mbhc_cb->hph_auto_pulldown_ctrl(mbhc->component,
								false);
	} else {
		/*
		...
		if (mbhc->micbias_enable && mbhc->micbias_enable_cb) {
			...
			mbhc->micbias_enable_cb(mbhc->component, true,
						mbhc->mbhc_cfg->micbias);
		}
...
static short wcd9xxx_read_sta_result(struct snd_soc_component *component)
{
	u8 bias_msb, bias_lsb;
	short bias_value;

	bias_msb = snd_soc_component_read(component, WCD9XXX_A_CDC_MBHC_B3_STATUS);
	bias_lsb = snd_soc_component_read(component, WCD9XXX_A_CDC_MBHC_B2_STATUS);
	bias_value = (bias_msb << 8) | bias_lsb;
	return bias_value;
}

static short wcd9xxx_read_dce_result(struct snd_soc_component *component)
{
	u8 bias_msb, bias_lsb;
	short bias_value;

	bias_msb = snd_soc_component_read(component, WCD9XXX_A_CDC_MBHC_B5_STATUS);
	bias_lsb = snd_soc_component_read(component, WCD9XXX_A_CDC_MBHC_B4_STATUS);
	bias_value = (bias_msb << 8) | bias_lsb;
	return bias_value;
}

static void wcd9xxx_turn_onoff_rel_detection(struct snd_soc_component *component,
					     bool on)
{
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B1_CTL, 0x02, on << 1);
}
				     ...
				     bool override_bypass, bool noreldetection)
{
	...
	if (noreldetection)
		wcd9xxx_turn_onoff_rel_detection(component, false);

	if (mbhc->mbhc_cfg->do_recalibration)
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x2,
				    0x0);
	/* Turn on the override */
	if (!override_bypass)
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B1_CTL, 0x4, 0x4);
	if (dce) {
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x8,
				    0x8);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_EN_CTL, 0x4);
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x8,
				    0x0);
		if (mbhc->mbhc_cfg->do_recalibration)
			snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL,
					    0x2, 0x2);
		...
		bias_value = wcd9xxx_read_dce_result(component);
	} else {
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x8,
				    0x8);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_EN_CTL, 0x2);
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x8,
				    0x0);
		if (mbhc->mbhc_cfg->do_recalibration)
			snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL,
		...
		bias_value = wcd9xxx_read_sta_result(component);
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x8,
				    0x8);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_EN_CTL, 0x0);
	}
	/* Turn off the override after measuring mic voltage */
	if (!override_bypass)
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B1_CTL, 0x04,
				    0x00);

	if (noreldetection)
		wcd9xxx_turn_onoff_rel_detection(component, true);
				   ...
				   bool norel)
{
	...
	override_bypass = (snd_soc_component_read(mbhc->component,
		...
		bool enable)
{
	if (enable) {
		...
		if (mbhc->mbhc_cb && mbhc->mbhc_cb->codec_rco_ctrl) {
			...
			mbhc->mbhc_cb->codec_rco_ctrl(mbhc->component, true);
		} else {
		...
		if (mbhc->mbhc_cb && mbhc->mbhc_cb->codec_rco_ctrl) {
			mbhc->mbhc_cb->codec_rco_ctrl(mbhc->component, false);
			WCD9XXX_BG_CLK_LOCK(mbhc->resmgr);
		} else {
			WCD9XXX_BG_CLK_LOCK(mbhc->resmgr);
				...
				bool is_cs_enable)
{
	...
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mb_source)
		mbhc->mbhc_cb->enable_mb_source(component, true, true);
	...
	snd_soc_component_update_bits(component, WCD9XXX_A_CLK_BUFF_EN1, 0x05, 0x01);

	/* Make sure CFILT is in fast mode, save current mode */
	cfilt_mode = snd_soc_component_read(component, mbhc_micb_regs->cfilt_ctl);
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->cfilt_fast_mode)
		mbhc->mbhc_cb->cfilt_fast_mode(component, mbhc);
	else
		snd_soc_component_update_bits(component, mbhc_micb_regs->cfilt_ctl,
				    0x70, 0x00);

	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x2, 0x2);
	snd_soc_component_write(component, WCD9XXX_A_MBHC_SCALING_MUX_1,
		      mbhc->scaling_mux_in);
	...
	snd_soc_component_update_bits(component, WCD9XXX_A_TX_7_MBHC_EN, 0x80, 0x80);
	snd_soc_component_update_bits(component, WCD9XXX_A_TX_7_MBHC_EN, 0x1F, 0x1C);
	snd_soc_component_update_bits(component, WCD9XXX_A_TX_7_MBHC_TEST_CTL, 0x40, 0x40);

	snd_soc_component_update_bits(component, WCD9XXX_A_TX_7_MBHC_EN, 0x80, 0x00);
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x8, 0x8);
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x8, 0x00);

	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B1_CTL, 0x2, 0x2);
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x8, 0x8);
	...
	snd_soc_component_write(component, mbhc_micb_regs->cfilt_ctl, cfilt_mode);
	snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_HPH, 0x13, 0x00);
				...
				bool is_cs_enable)
{
	...
	if (mbhc->mbhc_cfg->do_recalibration) {
		...
		reg = snd_soc_component_read(component, WCD9XXX_A_CDC_MBHC_B1_CTL);
		change = snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B1_CTL,
					     0x78, btn_det->mbhc_nsc << 3);
		wcd9xxx_get_z(mbhc, &dce_z, &sta_z, mbhc_micb_regs, true);
		if (change)
			snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_B1_CTL, reg);
		if (dce_z && sta_z) {
		...
		if (is_cs_enable) {
			...
			reg = snd_soc_component_read(mbhc->component,
					   WCD9XXX_A_CDC_MBHC_B1_CTL);
			...
			wcd9xxx_get_z(mbhc, &dce_z, NULL, mbhc_micb_regs,
				      true);
			snd_soc_component_write(mbhc->component, WCD9XXX_A_CDC_MBHC_B1_CTL,
				      reg);
			if (dce_z) {
...
static void wcd9xxx_shutdown_hs_removal_detect(struct wcd9xxx_mbhc *mbhc)
{
	...
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x2, 0x2);
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B1_CTL, 0x6, 0x0);
	__wcd9xxx_switch_micbias(mbhc, 0, false, false);

	usleep_range(generic->t_shutdown_plug_rem,
		     generic->t_shutdown_plug_rem + 50);

	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0xA, 0x8);
	...
	snd_soc_component_write(component, WCD9XXX_A_MBHC_SCALING_MUX_1, 0x00);
}
...
static void wcd9xxx_cleanup_hs_polling(struct wcd9xxx_mbhc *mbhc)
{
	...
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mb_source)
		mbhc->mbhc_cb->enable_mb_source(mbhc->component, false, true);
...
/* called under codec_resource_lock acquisition */
static void wcd9xxx_codec_hphr_gnd_switch(struct snd_soc_component *component, bool on)
{
	snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_HPH, 0x01, on);
	if (on)
...
static void wcd9xxx_onoff_vddio_switch(struct wcd9xxx_mbhc *mbhc, bool on)
{
	...
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->pull_mb_to_vddio) {
		mbhc->mbhc_cb->pull_mb_to_vddio(mbhc->component, on);
		goto exit;
	}

	if (on) {
		snd_soc_component_update_bits(mbhc->component, mbhc->mbhc_bias_regs.mbhc_reg,
				    1 << 7, 1 << 7);
		snd_soc_component_update_bits(mbhc->component, WCD9XXX_A_MAD_ANA_CTRL,
				    1 << 4, 0);
	} else {
		snd_soc_component_update_bits(mbhc->component, WCD9XXX_A_MAD_ANA_CTRL,
				    1 << 4, 1 << 4);
...
static int wcd9xxx_hphl_status(struct wcd9xxx_mbhc *mbhc)
{
	...
	WCD9XXX_BCL_ASSERT_LOCKED(mbhc->resmgr);
	hph = snd_soc_component_read(component, WCD9XXX_A_MBHC_HPH);
	snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_HPH, 0x12, 0x02);
	usleep_range(WCD9XXX_HPHL_STATUS_READY_WAIT_US,
		     WCD9XXX_HPHL_STATUS_READY_WAIT_US +
		     WCD9XXX_USLEEP_RANGE_MARGIN_US);
	status = snd_soc_component_read(component, WCD9XXX_A_RX_HPH_L_STATUS);
	snd_soc_component_write(component, WCD9XXX_A_MBHC_HPH, hph);
	return status;
}
...
static int wcd9xxx_pull_down_micbias(struct wcd9xxx_mbhc *mbhc, int us)
{
	 ...
	 */
	if ((snd_soc_component_read(mbhc->component, ctlreg) & 0x01)) {
		WARN_ONCE(1, "MBHC micbias is already pulled down unexpectedly\n");
		return -EFAULT;
	}

	if ((snd_soc_component_read(mbhc->component, WCD9XXX_A_MAD_ANA_CTRL) & 1 << 4)) {
		snd_soc_component_update_bits(mbhc->component, WCD9XXX_A_MAD_ANA_CTRL,
				    1 << 4, 0);
		micbiasconn = true;
	}

	snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.ctl_reg, 0x01, 0x01);
	...
	if (micbiasconn)
		snd_soc_component_update_bits(mbhc->component, WCD9XXX_A_MAD_ANA_CTRL,
				    1 << 4, 1 << 4);
	snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.ctl_reg, 0x01, 0x00);
				       ...
				       bool on, bool highhph)
{
	...
	if (on) {
		...
		snd_soc_component_update_bits(component, mbhc_micb_regs->mbhc_reg,
				    0x01, 0x00);
		/*
		 * Keep the low power insertion/removal
		 * detection (reg 0x3DD) disabled
		 */
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_INT_CTL,
				    0x01, 0x00);
		/*
		...
		snd_soc_component_update_bits(component, WCD9XXX_A_MAD_ANA_CTRL, 0x10, 0x00);
		mbhc->is_cs_enabled = true;
	} else {
		pr_debug("%s: disabling current source\n", __func__);
		/* Connect MBHC Override from MicBias and LDOH */
		snd_soc_component_update_bits(component, WCD9XXX_A_MAD_ANA_CTRL, 0x10, 0x10);
		...
		/* Nsc to acdb value */
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B1_CTL, 0x78,
				    btn_det->mbhc_nsc << 3);
...
wcd9xxx_codec_cs_get_plug_type(struct wcd9xxx_mbhc *mbhc, bool highhph)
{
	...
	WARN_ON_ONCE(NUM_DCE_PLUG_INS_DETECT < 4);
	...
	for (i = 1; i < NUM_DCE_PLUG_INS_DETECT - 1; i++) {
		...
		if (rt[i].swap_gnd)
			wcd9xxx_codec_hphr_gnd_switch(component, true);
		...
		if (rt[i].mic_bias)
			wcd9xxx_turn_onoff_current_source(mbhc,
							  ...
							  true, false);
		if (rt[i].swap_gnd)
			wcd9xxx_codec_hphr_gnd_switch(component, false);
	}
...
wcd9xxx_codec_get_plug_type(struct wcd9xxx_mbhc *mbhc, bool highhph)
{
	...
	WARN_ON(!(snd_soc_component_read(component, WCD9XXX_A_CDC_MBHC_B1_CTL) & 0x04));

	/* GND and MIC swap detection requires at least 2 rounds of DCE */
	WARN_ON_ONCE(NUM_DCE_PLUG_INS_DETECT < 2);
...
static bool wcd9xxx_swch_level_remove(struct wcd9xxx_mbhc *mbhc)
{
	if (mbhc->mbhc_cfg->gpio)
		...
		if (mbhc->mbhc_cb && mbhc->mbhc_cb->insert_rem_status)
			return mbhc->mbhc_cb->insert_rem_status(mbhc->component);
		else
...
static bool is_clk_active(struct snd_soc_component *component)
{
	return !!(snd_soc_component_read(component, WCD9XXX_A_CDC_CLK_MCLK_CTL) & 0x05);
}
				    ...
				    int insertion, int trigger, bool padac_off)
{
	...
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_INT_CTL, 0x1, 0);
	 ...
	 */
	snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.ctl_reg, 0x01, 0x01);
	snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.mbhc_reg, 0x90, 0x00);
	...
	if (insertion) {
		...
		if (trigger & MBHC_USE_HPHL_TRIGGER) {
			...
			snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_HPH, 0x11,
					0x11);
			snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_HPH, 0x0C,
					plug_det->hph_current << 2);
			snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_HPH, 0x02,
					0x02);
		}
		...
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_INT_CTL, 0x2, 0);
	} else {
		pr_debug("setup for removal detection\n");
		/* Make sure the HPH schmitt trigger is OFF */
		snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_HPH, 0x12, 0x00);

		/* enable the mic line schmitt trigger */
		snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.ctl_reg,
				    0x01, 0x00);
		snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.mbhc_reg, 0x60,
		...
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_INT_CTL, 0x2,
				    0x2);
	}

	if (snd_soc_component_read(component, WCD9XXX_A_CDC_MBHC_B1_CTL) & 0x4) {
		/* called by interrupt */
		if (!is_clk_active(component)) {
	...
	snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.int_rbias, 0x80, 0);

	/* If central bandgap disabled */
	if (!(snd_soc_component_read(component, WCD9XXX_A_PIN_CTL_OE1) & 1)) {
		snd_soc_component_update_bits(component, WCD9XXX_A_PIN_CTL_OE1, 0x3, 0x3);
	...
	/* If LDO_H disabled */
	if (snd_soc_component_read(component, WCD9XXX_A_PIN_CTL_OE0) & 0x80) {
		snd_soc_component_update_bits(component, WCD9XXX_A_PIN_CTL_OE0, 0x10, 0);
		snd_soc_component_update_bits(component, WCD9XXX_A_PIN_CTL_OE0, 0x80, 0x80);
		usleep_range(generic->t_ldoh, generic->t_ldoh +
					      WCD9XXX_USLEEP_RANGE_MARGIN_US);
		snd_soc_component_update_bits(component, WCD9XXX_A_PIN_CTL_OE0, 0x80, 0);

		if (central_bias_enabled)
			snd_soc_component_update_bits(component, WCD9XXX_A_PIN_CTL_OE1, 0x1,
					    0);
	}

	if (mbhc->resmgr->reg_addr && mbhc->resmgr->reg_addr->micb_4_mbhc)
		snd_soc_component_update_bits(component, mbhc->resmgr->reg_addr->micb_4_mbhc,
				    0x3, mbhc->mbhc_cfg->micbias);

	wcd9xxx_enable_irq(mbhc->resmgr->core_res, mbhc->intr_ids->insertion);
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_INT_CTL, 0x1, 0x1);
	return 0;
}
...
static bool wcd9xxx_detect_anc_plug_type(struct wcd9xxx_mbhc *mbhc)
{
	...
	override_en = (snd_soc_component_read(mbhc->component, WCD9XXX_A_CDC_MBHC_B1_CTL) &
		       0x04) ? true : false;
...
static irqreturn_t wcd9xxx_hs_insert_irq(int irq, void *data)
{
	...
	is_mb_trigger = !!(snd_soc_component_read(component, mbhc->mbhc_bias_regs.mbhc_reg) &
			   0x10);
	is_removal = !!(snd_soc_component_read(component, WCD9XXX_A_CDC_MBHC_INT_CTL) & 0x02);
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_INT_CTL, 0x03, 0x00);
...
static void wcd9xxx_btn_lpress_fn(struct work_struct *work)
{
	...
	bias_value = wcd9xxx_read_sta_result(mbhc->component);
	sta_mv = wcd9xxx_codec_sta_dce_v(mbhc, 0, bias_value);

	bias_value = wcd9xxx_read_dce_result(mbhc->component);
...
static void wcd9xxx_onoff_ext_mclk(struct wcd9xxx_mbhc *mbhc, bool on)
{
	...
	if (mbhc && mbhc->mbhc_cfg && mbhc->mbhc_cfg->mclk_cb_fn)
		mbhc->mbhc_cfg->mclk_cb_fn(mbhc->component, on, false);
}
...
static void wcd9xxx_correct_swch_plug(struct work_struct *work)
{
	...
	if (plug_type == PLUG_TYPE_HEADPHONE) {
		if (mbhc->mbhc_cb && mbhc->mbhc_cb->hph_auto_pulldown_ctrl)
			mbhc->mbhc_cb->hph_auto_pulldown_ctrl(component, true);
	}
...
static void wcd9xxx_swch_irq_handler(struct wcd9xxx_mbhc *mbhc)
{
	...
	if ((mbhc->current_plug == PLUG_TYPE_NONE) && insert) {
		...
		if ((mbhc->current_plug != PLUG_TYPE_NONE) &&
		    (mbhc->current_plug != PLUG_TYPE_HIGH_HPH) &&
		    !(snd_soc_component_read(component, WCD9XXX_A_MBHC_INSERT_DETECT) &
				   (1 << 1))) {
		...
		snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.ctl_reg, 0x01,
				    0x00);
		snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_HPH, 0x01, 0x00);
		wcd9xxx_mbhc_detect_plug_type(mbhc);
		...
		if (is_removed) {
			...
			snd_soc_component_update_bits(component,
					mbhc->mbhc_bias_regs.ctl_reg, 0x01,
					0x01);
			snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_HPH, 0x01,
					0x01);
			  ...
			  bool norel_detection)
{
	...
	reg0 = snd_soc_component_read(component, micb_regs->ctl_reg);
	snd_soc_component_update_bits(component, micb_regs->ctl_reg, 0x81, 0x1);
	reg1 = snd_soc_component_read(component, micb_regs->mbhc_reg);
	snd_soc_component_update_bits(component, micb_regs->mbhc_reg, 1 << 7, 0);

	/* Disconnect override from micbias */
	change = snd_soc_component_update_bits(component, WCD9XXX_A_MAD_ANA_CTRL, 1 << 4,
				     1 << 0);
	...
	if (change)
		snd_soc_component_update_bits(component, WCD9XXX_A_MAD_ANA_CTRL, 1 << 4,
				    1 << 4);
	/* Disable pull down micbias to ground */
	snd_soc_component_write(component, micb_regs->mbhc_reg, reg1);
	snd_soc_component_write(component, micb_regs->ctl_reg, reg0);
}
...
irqreturn_t wcd9xxx_dce_handler(int irq, void *data)
{
	...
	mbhc_status = snd_soc_component_read(component, WCD9XXX_A_CDC_MBHC_B1_STATUS) & 0x3E;
	...
	dce[0] = wcd9xxx_read_dce_result(component);
	sta = wcd9xxx_read_sta_result(component);
...
static irqreturn_t wcd9xxx_hphl_ocp_irq(int irq, void *data)
{
	...
	if (mbhc) {
		...
		if ((mbhc->hphlocp_cnt < OCP_ATTEMPT) &&
			...
			pr_info("%s: retry\n", __func__);
			mbhc->hphlocp_cnt++;
			snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_OCP_CTL,
					    0x10, 0x00);
			snd_soc_component_update_bits(component, WCD9XXX_A_RX_HPH_OCP_CTL,
					    0x10, 0x10);
		} else {
...
static void wcd9xxx_update_mbhc_clk_rate(struct wcd9xxx_mbhc *mbhc, u32 rate)
{
	...
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_TIMER_B1_CTL, n_ready[idx]);
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_TIMER_B6_CTL, ncic);
	...
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_TIMER_B2_CTL, npoll);
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_TIMER_B3_CTL, nbounce_wait);
}
...
static void wcd9xxx_mbhc_cal(struct wcd9xxx_mbhc *mbhc)
{
	 ...
	 */
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mb_source)
		mbhc->mbhc_cb->enable_mb_source(component, true, false);

	cfilt_mode = snd_soc_component_read(component, mbhc->mbhc_bias_regs.cfilt_ctl);
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->cfilt_fast_mode)
		mbhc->mbhc_cb->cfilt_fast_mode(component, mbhc);
	else
	...
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->select_cfilt)
		mbhc->mbhc_cb->select_cfilt(component, mbhc);
	else
		snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.ctl_reg, 0x60,
				mbhc->mbhc_cfg->micbias << 5);
	snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.ctl_reg, 0x01, 0x00);
	snd_soc_component_update_bits(component, WCD9XXX_A_LDO_H_MODE_1, 0x60, 0x60);
	snd_soc_component_write(component, WCD9XXX_A_TX_7_MBHC_TEST_CTL, 0x78);
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->codec_specific_cal)
		mbhc->mbhc_cb->codec_specific_cal(component, mbhc);
	else
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B1_CTL,
				    0x04, 0x04);

	/* Pull down micbias to ground */
	reg0 = snd_soc_component_read(component, mbhc->mbhc_bias_regs.ctl_reg);
	snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.ctl_reg, 1, 1);
	/* Disconnect override from micbias */
	reg1 = snd_soc_component_read(component, WCD9XXX_A_MAD_ANA_CTRL);
	snd_soc_component_update_bits(component, WCD9XXX_A_MAD_ANA_CTRL, 1 << 4, 1 << 0);
	/* Connect the MUX to micbias */
	snd_soc_component_write(component, WCD9XXX_A_MBHC_SCALING_MUX_1, 0x02);
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mux_bias_block)
		mbhc->mbhc_cb->enable_mux_bias_block(component);
	else
		snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_SCALING_MUX_1,
	...
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x0A);
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x02);
	mbhc->mbhc_data.dce_z = __wcd9xxx_codec_sta_dce(mbhc, 1, true, false);

	/* compute dce_z for current source */
	reg2 = snd_soc_component_read(component, WCD9XXX_A_CDC_MBHC_B1_CTL);
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B1_CTL, 0x78,
	...
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_B1_CTL, reg2);
	...
	/* Restore registers */
	snd_soc_component_write(component, mbhc->mbhc_bias_regs.ctl_reg, reg0);
	snd_soc_component_write(component, WCD9XXX_A_MAD_ANA_CTRL, reg1);
	 ...
	 */
	msleep(WCD9XXX_MUX_SWITCH_READY_WAIT_MS);
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_EN_CTL, 0x04);
	usleep_range(mbhc->mbhc_data.t_dce, mbhc->mbhc_data.t_dce +
					WCD9XXX_USLEEP_RANGE_MARGIN_US);
	mbhc->mbhc_data.dce_mb = wcd9xxx_read_dce_result(component);

	/* STA Measurement for MB Voltage */
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x0A);
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_EN_CTL, 0x02);
	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_CLK_CTL, 0x02);
	snd_soc_component_write(component, WCD9XXX_A_MBHC_SCALING_MUX_1, 0x02);
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mux_bias_block)
		mbhc->mbhc_cb->enable_mux_bias_block(component);
	...
	usleep_range(mbhc->mbhc_data.t_sta, mbhc->mbhc_data.t_sta +
					WCD9XXX_USLEEP_RANGE_MARGIN_US);
	mbhc->mbhc_data.sta_mb = wcd9xxx_read_sta_result(component);

	/* Restore default settings. */
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B1_CTL, 0x04, 0x00);
	snd_soc_component_write(component, mbhc->mbhc_bias_regs.cfilt_ctl, cfilt_mode);
	snd_soc_component_write(component, WCD9XXX_A_MBHC_SCALING_MUX_1, 0x04);
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mux_bias_block)
		mbhc->mbhc_cb->enable_mux_bias_block(component);
	else
	...
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mb_source)
		mbhc->mbhc_cb->enable_mb_source(component, false, false);
...
static void wcd9xxx_mbhc_setup(struct wcd9xxx_mbhc *mbhc)
{
	...
	for (n = 0; n < 8; n++) {
		snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_FIR_B1_CFG,
				    0x07, n);
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_FIR_B2_CFG,
			      btn_det->c[n]);
	}

	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B2_CTL, 0x07,
			    btn_det->nc);

	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_TIMER_B4_CTL, 0x70,
			    generic->mbhc_nsa << 4);

	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_TIMER_B4_CTL, 0x0F,
			    btn_det->n_meas);

	snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_TIMER_B5_CTL,
		      generic->mbhc_navg);

	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B1_CTL, 0x80, 0x80);
	...
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B1_CTL, 0x02, 0x02);

	snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_SCALING_MUX_2, 0xF0, 0xF0);

	gain = wcd9xxx_mbhc_cal_btn_det_mp(btn_det, MBHC_BTN_DET_GAIN);
	snd_soc_component_update_bits(component, WCD9XXX_A_CDC_MBHC_B2_CTL, 0x78,
			    gain[idx] << 3);
	snd_soc_component_update_bits(component, WCD9XXX_A_MICB_2_MBHC, 0x04, 0x04);
...
static int wcd9xxx_setup_jack_detect_irq(struct wcd9xxx_mbhc *mbhc)
{
	...
	if (mbhc->mbhc_cfg->gpio) {
		...
		/* Enable HPHL_10K_SW */
		snd_soc_component_update_bits(mbhc->component, WCD9XXX_A_RX_HPH_OCP_CTL,
				    1 << 1, 1 << 1);
...
static int wcd9xxx_init_and_calibrate(struct wcd9xxx_mbhc *mbhc)
{
	...
	/* Enable Mic Bias pull down and HPH Switch to GND */
	snd_soc_component_update_bits(component, mbhc->mbhc_bias_regs.ctl_reg, 0x01, 0x01);
	snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_HPH, 0x01, 0x01);
...
static void wcd9xxx_mbhc_fw_read(struct work_struct *work)
{
	...
	while (retry < FW_READ_ATTEMPTS) {
		...
		if (mbhc->mbhc_cb->get_hwdep_fw_cal)
			fw_data = mbhc->mbhc_cb->get_hwdep_fw_cal(component,
					WCD9XXX_MBHC_CAL);
		if (!fw_data)
	...
	if (!use_default_cal) {
		...
		if (!wcd9xxx_mbhc_fw_validate(data, size)) {
...
{
	mbhc->debugfs_poke =
	    debugfs_create_file("TRRS", 0444,/*S_IFREG | S_IRUGO,*/ NULL, mbhc,
				&mbhc_trrs_debug_ops);
	mbhc->debugfs_mbhc =
	    debugfs_create_file("wcd9xxx_mbhc", 0444,/*S_IFREG | S_IRUGO,*/
		       ...
		       struct wcd9xxx_mbhc_config *mbhc_cfg)
{
	...
	if (mbhc->mbhc_cb &&
		...
		mbhc->mbhc_cb->enable_clock_gate(mbhc->component, true);
...
EXPORT_SYMBOL_GPL(wcd9xxx_mbhc_stop);
...
static void wcd9xxx_enable_mbhc_txfe(struct wcd9xxx_mbhc *mbhc, bool on)
{
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mbhc_txfe)
		mbhc->mbhc_cb->enable_mbhc_txfe(mbhc->component, on);
	else
		snd_soc_component_update_bits(mbhc->component, WCD9XXX_A_TX_7_MBHC_TEST_CTL,
				    0x40, on ? 0x40 : 0x00);
}
				...
				void *data)
{
	...
	case WCD9XXX_EVENT_POST_MICBIAS_4_OFF:
		...
		if (mbhc->micbias_enable && mbhc->polling_active &&
		    !(snd_soc_component_read(mbhc->component, mbhc->mbhc_bias_regs.ctl_reg)
		    & 0x80)) {
	...
	case WCD9XXX_EVENT_PRE_HPHL_PA_ON:
		...
		if (!(snd_soc_component_read(component, mbhc->mbhc_bias_regs.ctl_reg) & 0x80))
	...
	case WCD9XXX_EVENT_POST_MCLK_ON:
		...
		snd_soc_component_update_bits(component, WCD9XXX_A_TX_COM_BIAS, 1 << 4,
				    1 << 4);
	...
	case WCD9XXX_EVENT_PRE_TX_3_ON:
		...
		if (!(snd_soc_component_read(component, mbhc->mbhc_bias_regs.ctl_reg)
		      & 0x80) &&
...
static s16 wcd9xxx_read_impedance_regs(struct wcd9xxx_mbhc *mbhc)
{
	...
	for (i = 0; i < z_loop; i++) {
		snd_soc_component_write(component, WCD9XXX_A_CDC_MBHC_EN_CTL, 0x2);
		/* Wait for at least 1800uS to let register write to settle */
		usleep_range(1800, 1800 + WCD9XXX_USLEEP_RANGE_MARGIN_US);
		z_t += wcd9xxx_read_sta_result(component);
	}
					...
					u32 *zl_stereo, u32 *zl_mono)
{
	...
	if (!mbhc->mbhc_cb || !mbhc->mbhc_cb->setup_zdet ||
		...
		dev_err(component->dev, "%s: Invalid parameters\n", __func__);
		return -EINVAL;
	}
	...
	dev_dbg(component->dev, "%s: Remeasuring impedance values\n", __func__);
	dev_dbg(component->dev, "%s: l: %p, r: %p, left=%d, right=%d\n", __func__,
		 l, r, left, right);

	/* Remeasure V2 values */
	snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_SCALING_MUX_2, 0xFF, 0xF0);
	if (right)
		r_t[2] = wcd9xxx_read_impedance_regs(mbhc);
	snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_SCALING_MUX_1, 0xFF, 0xC0);
	if (left)
	...
	if (right) {
		/* Take R0'/R1' */
		snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_SCALING_MUX_2,
				    0xFF, 0xF8);
		snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_SCALING_MUX_1,
				    0xFF, 0xA0);
	...
	snd_soc_component_update_bits(component, WCD9XXX_A_MBHC_SCALING_MUX_2, 0xFF, 0xF8);
...
//	mutex_lock(&codec->mutex);
	 ...
	 */
	micb_mbhc_val = snd_soc_component_read(component, WCD9XXX_A_MAD_ANA_CTRL);
	snd_soc_component_update_bits(component, WCD9XXX_A_MAD_ANA_CTRL,
			    0x10, 0x00);

	override_en = (snd_soc_component_read(component, WCD9XXX_A_CDC_MBHC_B1_CTL) & 0x04) ?
					true : false;
	if (!override_en)
		wcd9xxx_turn_onoff_override(mbhc, true);
	...
	for (i = 0; i < ARRAY_SIZE(reg_set_mux) - 2; i++) {
		snd_soc_component_update_bits(component, reg_set_mux[i].reg,
				    reg_set_mux[i].mask,
				    reg_set_mux[i].val);
...
//	mutex_unlock(&codec->mutex);
	...
	snd_soc_component_write(component, WCD9XXX_A_MAD_ANA_CTRL, micb_mbhc_val);
			       ...
			       uint32_t *zr)
{
	...
	*zr = mbhc->zr;
	...
	if (*zl && *zr)
		...
		return -EINVAL;
} EXPORT_SYMBOL(wcd9xxx_mbhc_get_impedance);
...
int wcd9xxx_mbhc_init(struct wcd9xxx_mbhc *mbhc, struct wcd9xxx_resmgr *resmgr,
		      struct snd_soc_component *component,
		      int (*micbias_enable_cb)(struct snd_soc_component *component,  bool,
		      ...
		      bool impedance_det_en)
{
	...
	if (mbhc->headset_jack.jack == NULL) {
		ret = snd_soc_card_jack_new(component->card, "Headset Jack", WCD9XXX_JACK_MASK,
	 ...
	 *		wcd9xxx_dce_handler, "DC Estimation detect",mbhc);
	 * if (ret) {
	 *	pr_err("%s: Failed to request irq %d\n", __func__,
	 *			mbhc->intr_ids->dce_est_complete);
