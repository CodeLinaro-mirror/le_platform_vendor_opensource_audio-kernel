...
extern void wcd9xxx_clsh_fsm(struct snd_soc_component *component,
		struct wcd9xxx_clsh_cdc_data *cdc_clsh_d,
		u8 req_state, bool req_type, u8 clsh_event);

extern void wcd9xxx_enable_high_perf_mode(struct snd_soc_component *component,
...
extern void wcd9xxx_clsh_imped_config(struct snd_soc_component *component,
				  int imped);
...
extern int wcd9xxx_soc_update_bits_push(struct snd_soc_component *component,
					struct list_head *lh,
					uint16_t reg, uint8_t mask,
					uint8_t value, int delay);
extern void wcd9xxx_restore_registers(struct snd_soc_component *component,
				      struct list_head *lh);
enum {
	RESERVED = 0,
	AANC_LPF_FF_FB = 1,
